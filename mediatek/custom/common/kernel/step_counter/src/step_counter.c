...
static void step_c_work_func(struct work_struct *work)
{
	...
	err = cxt->step_c_data.get_data(&value,&status);    //henry: == step_counter_get_data()
...
static void step_c_poll(unsigned long data)
{
	...
	if(obj != NULL)
	{
    Printhh("[%s] call schedule_work()..\n", __FUNCTION__);
		schedule_work(&obj->report);    // == step_c_work_func()
	}
}
                                  ...
                                  const char *buf, size_t count)
{
	...
	mutex_unlock(&step_c_context_obj->step_c_op_mutex);
	return count;//add by henry; fix for coverity
}
...
int step_c_register_data_path(struct step_c_data_path *data)
{
	...
	int err =0;
	cxt = step_c_context_obj;
	cxt->step_c_data.get_data = data->get_data; //henry: == step_counter_get_data()
	cxt->step_c_data.vender_div = data->vender_div;
	cxt->step_c_data.get_data_significant = data->get_data_significant;
	cxt->step_c_data.get_data_step_d = data->get_data_step_d;   //henry: == step_detect_get_data()
...
int step_c_data_report(struct input_dev *dev, int value, int status)
{
	...
	input_sync(dev); 
  	return 1;//add by henry; fix for coverity
}
