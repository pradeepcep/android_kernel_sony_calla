...
static int temp_V_buf[50]={0};
static int temp_v_sum = 0; 
static int NewSensi[3][2] = {{1024,1024},
			     {1024,1024},
			     {1024,1024}};
...
//input data array, do some operation then output data array for final g-sensor data
	Accl_Status=0x07; //
       for (i=0;i<KXTJ2_1009_AXES_NUM;i++)
       {
       	...
       	raw[i] = data[i];
          	 if (abs(raw[i]) < Offset_torlance)
               Accl_Status &= ~( 0x01 << i); 
       }
  
       if (( Accl_Status == 0x1 ) ||  ( Accl_Status == 0x2 ) || ( Accl_Status == 0x4 ))
	{
		Axis_K = raw[0]* (Accl_Status &     0x01) + 
		raw[1]*((Accl_Status & 	0x02)	>> 0x01) + 
		raw[2]*((Accl_Status & 	0x04)	>> 0x02);
		if ((abs((abs(Axis_K)- Sensitivity_def))  < (Offset_torlance + Sensitivity_torlance)))
		{
			...
			{
				temp_V_buf[k] = temp_V_buf[k+1];
				if (temp_V_buf[k] == 0) temp_V_buf[k] = Sensitivity_def ;
				temp_v_sum += temp_V_buf[k];
				if (temp_V_buf[k] > Wave_Max) Wave_Max = temp_V_buf[k];
				if (temp_V_buf[k] < Wave_Min) Wave_Min = temp_V_buf[k];
			}
			temp_V_buf[k] = Axis_K; 
			temp_v_sum += temp_V_buf[k];
			if (temp_V_buf[k] > Wave_Max) 
				Wave_Max = temp_V_buf[k];
			if (temp_V_buf[k] < Wave_Min) 
				Wave_Min = temp_V_buf[k];
			...
			if (Wave_Max-Wave_Min < Stable_range )
			{
				 if (Accl_Status ==4 ) 
				 	Accl_Status =3;
				if ( temp_v_sum > 0) 
					NewSensi[Accl_Status-1][0] = temp_v_sum / BUF_RANGE;
				else
					NewSensi[Accl_Status-1][1] = temp_v_sum / BUF_RANGE;
			}

		}
		else
		{ 
			temp_V_buf[BUF_RANGE-2] = NewBuffer; 
		}	
	}
	else{ 
		temp_V_buf[BUF_RANGE-2] = NewBuffer; 
	...
	for (i=0;i<3;i++)
	{
		if(raw[i]>=0)
		{
			raw[i] = raw[i] * 1024 / abs(NewSensi[i][0]);

		}
		else
		{
			raw[i] = raw[i] * 1024 / abs(NewSensi[i][1]);
		}
       }
	for (i=0;i<KXTJ2_1009_AXES_NUM;i++)
	{
    ...
    return 0;   //add by henry, fix for coverity
}
#endif
...
static int KXTJ2_1009_SCP_SetPowerMode(bool enable)
{
	...
	GSE_LOG("KXTJ2_1009_SCP_SetPowerMode %d!\n ",enable);
...
static int kxtj2_1009_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj_i2c_data = obj;
	obj->client = client;
	obj->client->timing = 400; // change I2C clock to 400khz.
...
#endif
	Printhh("[%s] ctl.is_support_batch=%d\n", __FUNCTION__, ctl.is_support_batch);
	...
	exit:
	kfree(obj);
	//exit: //add by henry; fix for coverity
	GSE_ERR("%s: err = %d\n", __func__, err);
