...
# define CTP_RST (GPIO144 | 0x80000000)
...
static s32 synaptics_rmi4_probe(struct i2c_client *client,const struct i2c_device_id *dev_id);
 ...
 static s32 synaptics_rmi4_probe(struct i2c_client *client,const struct i2c_device_id *dev_id)
{
  ...
  syna_info("liyibo : %s %d\n" , __func__, __LINE__);
	...
	mt_set_gpio_pull_select(GPIO_CTP_EINT_PIN, GPIO_PULL_UP);
	  syna_info("liyibo : %s %d\n" , __func__, __LINE__);	
	// power up sequence ruo mark
	hwPowerOn(MT6325_POWER_LDO_VGP1 , VOL_2800, "TP");

        msleep(12);
        mt_set_gpio_mode(CTP_RST, GPIO_CTP_RST_PIN_M_GPIO);
	mt_set_gpio_dir(CTP_RST, GPIO_DIR_OUT);
	mt_set_gpio_out(CTP_RST, GPIO_OUT_ZERO);
        msleep(50);
	mt_set_gpio_out(CTP_RST, GPIO_OUT_ONE);
        msleep(50);
...
extern int cci_det_hw_id_forTP(void);
// Celia end
...
static int __init synaptics_rmi4_init(void)
{
	// Celia start
	// Using two TP driver due to PDP1 (EVT1) use Synaptics, PDP2 (EVT2) use ELAN.
	printk("[synaptics] %s init synaptics\n", __func__);

	int cci_hw_id = 9; // No phase is num 9.
	cci_hw_id = cci_det_hw_id_forTP();
	printk("[synaptics] cci_hw_id:%d, ",cci_hw_id);

	if(cci_hw_id != 1) {
		printk("After Phase EVT1, TP do not use Synaptics\n");
		return 0;
	} else {
		printk("Phase EVT1, TP use Synapatics\n");
		// Celia end
