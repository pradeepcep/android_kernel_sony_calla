...
int stk3x1x_cci_als_cali(int sSampleNo)
{
	...
	// 20141201 Set range for LY28 IR ink
	if(als_reading <= 153 || als_reading >= 806){
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	if(diff > STK_MAX_MIN_DIFF && diff > temp_max_min_diff)
	{
		obj->psi_set = obj->psi;
		fac_ct = cci_result_ct;// 2*cci_ps_low_thd-cci_ps_high_thd;		
		APS_LOG("%s: STK_MAX_MIN_DIFF=%d, diff=%d, psi_set=%d, fac_ct=%d\n", __FUNCTION__, STK_MAX_MIN_DIFF, diff, obj->psi_set, fac_ct);
		//STK add for CCI start 20130112. Adjust for LY28 20150128
		if ((MAX_COMPARE(stk3x1x_obj->psi_set, fac_ct)) > (fac_ct*15/10))
			stk3x1x_obj->psi_set = (fac_ct*15/10);
		else
			stk3x1x_obj->psi_set = MAX_COMPARE(stk3x1x_obj->psi_set, fac_ct);
		//STK add for CCI start 20130112. Adjust for LY28 20150128
		APS_LOG("%s: new psi_set=%d\n", __FUNCTION__, stk3x1x_obj->psi_set);

		//STK add for CCI start 20130112
		if(cci_ps_low_thd==0 || cci_ps_high_thd==0)
		{
			APS_ERR("Use STK default value. Set HT=%d, LT=%d\n", (obj->psi_set + STK_HT_N_CT), (obj->psi_set + STK_LT_N_CT));
			atomic_set(&obj->ps_high_thd_val, obj->psi_set + STK_HT_N_CT); 
			atomic_set(&obj->ps_low_thd_val, obj->psi_set + STK_LT_N_CT); 		
		}
		else
		{
			APS_ERR("Use CCI calibration value. Set HT=%d, LT=%d\n", (obj->psi_set + cci_ps_high_thd-fac_ct), (obj->psi_set + cci_ps_low_thd-fac_ct));
...
static ssize_t stk3x1x_show_BC_ct(struct device_driver *ddri, char *buf)
{
	...
	if ((cci_result_ct < 2500) && (cci_ps_high_thd > (cci_result_ct*2))) { // STK modify for LY28 BC 0cm 20150128
		...
		if(Diff_ThdH_CT > 100){
			cci_ps_low_thd = cci_result_ct*18/10; //STK modify for LY28 BC 0cm 20150128
...
static ssize_t stk3x1x_show_test_BC(struct device_driver *ddri, char *buf)
{
	...
	APS_ERR("[#23]ps_CCI_test_BC_show() CCI_FTM_INTERRUPT = %d\n", CCI_FTM_INTERRUPT);
	APS_ERR("[#23] adc= %d\n",test_ps_code);
...
static ssize_t stk3x1x_show_test_Light(struct device_driver *ddri, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "cci_als_value = %d\n", cci_als_value);
}
...
// add for FTM interrupt check 20130416 end		
		...
		case ALSPS_IOCTL_SET_CALI_CCI:
			...
			APS_ERR("%s ALSPS_IOCTL_SET_CALI_CCI cci_result_ct = %d, cci_ps_cover = %d, cci_als_value = %d\n", __FUNCTION__, cci_result_ct, cci_ps_cover, cci_als_value);
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if((err = stk3x1x_create_attr(&stk3x1x_alsps_driver.driver)))
	{
		...
		APS_LOG("create attribute success");
	}
...
static int stk3x1x_probe(struct platform_device *pdev) 
{
	...
	APS_LOG("%s, linux version small than 3.0.0",__func__);
#endif
