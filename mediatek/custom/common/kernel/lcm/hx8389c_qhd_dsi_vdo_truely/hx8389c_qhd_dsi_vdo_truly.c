...
unsigned int lcm_esd_test2 = FALSE;      ///only for ESD test
...
static void lcm_init(void)
{
...
#ifdef BUILD_LK
			  printf("[LK]------LCM hx8389c000----%s------\n",__func__);
    #else
			  printk("[KERNEL]------LCM hx8389c000----%s------\n",__func__);
    #endif
    ...
    MDELAY(10);//Must over 6 ms,SPEC request
	...
	#ifdef BUILD_LK
		  printf("[LK]------LCM hx8389c----%s------\n",__func__);
    #else
		  printk("[KERNEL]------LCM hx8389c----%s------\n",__func__);
    #endif
		...
		//0xBF, 0x05 0x40 0x00/ 0x00 0x00 0x0C 0x60
		data_array[0] = 0x00083902;
		data_array[1] = 0x004005BF;
		data_array[2] = 0x600C0000;
		dsi_set_cmdq(data_array, 3, 1);

		//29 BA 41 83 A8 /4D D2 24 00/ 00 00 90 00/ 00
		//data_array[0] = 0x000D3902;
		//data_array[1] = 0xA88341BA;
		//data_array[2] = 0x0024D24D;
		//data_array[3] = 0x00900000;
		//data_array[4] = 0x00000000;
		//dsi_set_cmdq(data_array, 5, 1);
		/*cmd 3 0xB1,0x7F,0x10,0x10,0x32,0x32,
   	                0x50,0x10,0xF2,0x58,0x80,
   	                0x20,0x20,0xF8,0xAA,0xAA,
   	                0xA0,0x00,0x80,0x30,0x00 (part 1)

			0xB1,0x7F,0x10,0x10,0xF2,0x32,
   	                                               	   0x50,0x10,0xF2,0x58,0x80,
   	                                                   0x20,0x20,0xF8,0xAA,0xAA,
   	                                                   0xA0,0x00,0x80,0x30,0x00

			B1 7F 10 10 /f2 32 90 10/ EC 52 80 20 /20 F8 AA AA/ A1 00 80 30 /00 (part 2)
		*/
		...
		data_array[0] = 0x00153902;
		data_array[1] = 0x10107FB1;
		data_array[2] = 0x109032F2;
		data_array[3] = 0x208052EC;
		data_array[4] = 0xAAAAF820;
		data_array[5] = 0x308000A1;
		data_array[6] = 0x00000000;
		dsi_set_cmdq(data_array, 7, 1);
		*/
#ifdef BUILD_LK
			  printf("[LK]=== lcm hx8389c init set cmd1 ===\n",__func__);
    #else
			  printk("[KERNEL]=== lcm hx8389c init set cmd1 ===\n",__func__);
    #endif
	
		/*cmd 4 
		0xB2,0x80,0x50,0x05,0x07,0x40,0x38,0x11,0x64,0x5D,0x09
		B2 80 50 05 07 40 38 11 64 5D 09 (part 2)
		*/
...
#ifdef BUILD_LK
					  printf("[LK]=== === lcm hx8389c init set cmd2 ===\n",__func__);
    #else
					  printk("[KERNEL]=== lcm hx8389c init set cmd2 ===\n",__func__);
    #endif

	
		/*cmd 5
		0xB4,0x70,0x70,0x70/,0x70,0x00,0x00,0x10,0x76,0x10,0x76,0xB0 
		B4 70 70 70 /70 00 00 10 /56 10 56 B0 (part 2)
		*/
		...
		data_array[1] = 0x707070B4;
		data_array[2] = 0x10000070;
		data_array[3] = 0xB0561056;
		dsi_set_cmdq(data_array, 4, 1);
		*/
		//printk("=== lcm hx8389b init set cmd3 ===");
		...
		D3 00 00 00 00 00 08 00 32 10 00 00 00 03 C6 03 C6 00 00 00 00 35 33 04 04 37 00 00 00 05 08 00 00 0A 00 01
		*/
		...
		D5 18 18 18 18 19 19 18 18 20 21 24 25 18 18 18 18 00 01 04 05 02 03 06 07 18 18 18 18 18 18 18 18 18 18 18 18 18 18
		*/
		...
		D6 18 18 18 18 18 18 19 19 25 24 21 20 18 18 18 18 07 06 03 02 05 04 01 00 18 18 18 18 18 18 18 18 18 18 18 18 18 18
		*/
		data_array[0] = 0x00273902;
		...
		*/
		...
		data_array[2] = 0x1C3F3A33;
		data_array[3] = 0x0E0B0842;
		data_array[4] = 0x15120F18;
		data_array[5] = 0x11061413;
		data_array[6] = 0x05001813;
		data_array[7] = 0x3F3A3307;
		data_array[8] = 0x0B08421C;
		data_array[9] = 0x120E180E;
		...
		*/
		...
		/*cmd 12
		-->PDP2
							   0xE0,0x00,0x0F,0x16/,0x35,0x3B,
	                                                   0x3F,0x21/,0x43,0x07,0x0B,
	                                                   0x0D/,0x18,0x0E,0x10,0x12/,
	                                                   0x11,0x13,0x06,0x10/,0x13,
	                                                   0x18,0x00,0x0F/,0x15,0x35,
	                                                   0x3B,0x3F/,0x21,0x42,0x07,
	                                                   0x0B/,0x0D,0x18,0x0D,0x11/,
	                                                   0x13,0x11,0x12,0x07/,0x11,0x12,0x17

		-->After SP
							   0xE0,0x00,0x0A,0x0F/,0x35,0x3B,
	                                                   0x3F,0x1F/,0x43,0x05,0x0A,
	                                                   0x0C/,0x17,0x0F,0x11,0x13/,
	                                                   0x12,0x13,0x06,0x10/,0x13,
	                                                   0x18,0x00,0x0A/,0x0E,0x35,
	                                                   0x3B,0x3F/,0x1F,0x42,0x05,
	                                                   0x0A/,0x0C,0x17,0x0E,0x12/,
	                                                   0x14,0x12,0x12,0x07/,0x11,0x12,0x17

		--> Latest Gamma
							   0xE0,0x00,0x17,0x1C/,0x33,0x3A,
							   0x3F,0x26,0x42,0x07,0x0A,
							   0x0C,0x17,0x0F,0x12,0x14,
							   0x13,0x14,0x06,0x11,0x13,
							   0x18,0x00,0x17,0x1C,0x33.
							   0x3A,0x3F,0x26,0x42,0x07,
							   0x0A,0x0C,0x17,0x0F,0x12.
							   0x14,0x13,0x14,0x06,0x11,0x13,0x18
		*/

	  if(hw_version == 0)
	  {
		#ifdef BUILD_LK
			printf("[LK]=== === PDP2 OLD glass ===\n",__func__);
    		#else
			printk("[KERNEL]=== PDP2 OLD glass ===\n",__func__);
    		#endif
		
		data_array[0] = 0x002B3902;
		data_array[1] = 0x160F00E0;
		data_array[2] = 0x213F3B35;
		data_array[3] = 0x0D0B0743;
		data_array[4] = 0x12100E18;
		data_array[5] = 0x10061311;
		data_array[6] = 0x0F001813;
		data_array[7] = 0x3F3B3515;
		data_array[8] = 0x0B074221;
		data_array[9] = 0x110D180D;
	  ...
	  {
		#ifdef BUILD_LK
			printf("[LK]=== === After SP NEW glass ===\n",__func__);
    		#else
			printk("[KERNEL]=== After SP NEW glass ===\n",__func__);
    		#endif
		/*
		data_array[0] = 0x002B3902;
		...
		*/
		data_array[0] = 0x002B3902;
		data_array[1] = 0x1C1700E0;
		data_array[2] = 0x263F3A33;
		data_array[3] = 0x0C0A0742;
		data_array[4] = 0x14120F17;
		data_array[5] = 0x11061413;
		data_array[6] = 0x17001813;
		data_array[7] = 0x3F3A331C;
		data_array[8] = 0x0A074226;
		data_array[9] = 0x120F170C;
		data_array[10] = 0x06141314;
		data_array[11] = 0x00181311;
		dsi_set_cmdq(data_array, 12, 1);
	  }	
		/*cmd 13
...
#ifdef BUILD_LK
							  printf("[LK]=== === lcm hx8389c init set done ===\n",__func__);
    #else
							  printk("[KERNEL]=== lcm hx8389c init set done ===\n",__func__);
    #endif
		//CABC
...
static unsigned int lcm_compare_id(void)
{
	...
	#ifdef BUILD_LK
	printf("%s, ruo lcm 8389\n", __func__);
#else
	printk("%s, ruo lcm 8389 \n",__func__);
#endif

			//printk("=== lcm hx8389c init ===");
		/**
...
//{0x39,0xBA,7,{0x41,0x93,0x00,0x16,0xA4,0x10,0x18}},	
	...
	{
		printf("[LK]=== === lcm hx8389c main source\n");
		return 1; //Main source
	}
	else 
		return 0;
}
...
static void lcm_suspend(void)
{
	...
	**/
	...
	//-->SLPIN_Solution
	//B0, 00 , 01 , 00 , 0C , 1F , 00
	data_array[0] = 0x00073902;
	data_array[1] = 0x000100B0;
	data_array[2] = 0x00001F0C;
	dsi_set_cmdq(data_array, 3, 1);

	MDELAY(5); 	
	//BF, 05 , 40 , 00 /, 00 , 00 , 0C , 1B/ , FF , 80
	data_array[0] = 0x000A3902;
	data_array[1] = 0x004005BF;
	data_array[2] = 0x1B0C0000;
	data_array[3] = 0x000080FF;
	dsi_set_cmdq(data_array, 4, 1);

	MDELAY(5);

	//B0, 00 , 01 , 00 , 00 , 1F , 00
	data_array[0] = 0x00073902;
	data_array[1] = 0x000100B0;
	data_array[2] = 0x00001F00;
	dsi_set_cmdq(data_array, 3, 1);

	MDELAY(5); 	

	//B0, 00 , 01 , 00/ , 00 , 00 , 00
	data_array[0] = 0x00073902;
	data_array[1] = 0x000100B0;
	...
	//B0, 00 , 00 , 01 , 0C , 00 , 00
	data_array[0] = 0x00073902;
	data_array[1] = 0x010000B0;
...
LCM_DRIVER hx8389c_qhd_dsi_vdo_truely_drv =
{
    .name			= "hx8389c_qhd_dsi_vdo_truely",
