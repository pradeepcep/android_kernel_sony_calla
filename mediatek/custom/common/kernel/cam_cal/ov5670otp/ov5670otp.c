...
static int read_otp_data(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   int temp1=0, temp_test=0;
   int otp_flag, addr;
   int i4RetValue = 0;
   int i;
   ...
   iReadData(0x5002, I2C_UNIT_SIZE, &temp1);
   *pOneByteBuff = (temp1&(~0x08));
   iWriteData(0x5002, I2C_UNIT_SIZE, pOneByteBuff );

   *pOneByteBuff = 0xc0;
   iWriteData(0x3d84, I2C_UNIT_SIZE, pOneByteBuff );
   *pOneByteBuff = 0x70;
   iWriteData(0x3d88, I2C_UNIT_SIZE, pOneByteBuff );
   *pOneByteBuff = 0x10;
   iWriteData(0x3d89, I2C_UNIT_SIZE, pOneByteBuff );
   *pOneByteBuff = 0x70;
   iWriteData(0x3d8a, I2C_UNIT_SIZE, pOneByteBuff );
   *pOneByteBuff = 0x31;
   iWriteData(0x3d8b, I2C_UNIT_SIZE, pOneByteBuff );
   *pOneByteBuff = 0x01;
   iWriteData(0x3d81, I2C_UNIT_SIZE, pOneByteBuff );
   mdelay(5);

/*
   for (i=0x7010; i<=0x7031; i++)//Should be removed when using Liteon module
   {
       temp_test = 0;
       iReadData(i, I2C_UNIT_SIZE, &temp_test );
       CAM_CALDB("[CAM_CAL][read_otp_data] Address 0x%x data value is 0x%x\n", i, temp_test);
   }
*/

   iReadData(0x7010, I2C_UNIT_SIZE, &otp_flag);
   CAM_CALDB("[CAM_CAL][read_otp_data] Address 0x7010 data value is 0x%x\n", otp_flag);
   addr = 0;
   ...
   CAM_CALDB("[CAM_CAL][read_otp_data] Group start address is 0x%x\n", addr);
   
   if (addr != 0)
   {
       i4RetValue = iReadData(ui4_offset+addr, ui4_length, pinputdata);
       CAM_CALDB("[CAM_CAL][read_otp_data] WorkingBuf length is %d\n", ui4_length);	   
       CAM_CALDB("[CAM_CAL][read_otp_data] WorkingBuf[4]= [0x%x 0x%x 0x%x 0x%x]\n", pinputdata[0], pinputdata[1], pinputdata[2], pinputdata[3]);	   
   }
   else
   {
       CAM_CALDB("[CAM_CAL][read_otp_data] No valid OTP data!\n");
       i4RetValue = -EFAULT;
   }
   ...
   {
       *pOneByteBuff = 0x00;
       iWriteData(i, I2C_UNIT_SIZE, pOneByteBuff );
   }
...
#endif
{
    ...
    switch(a_u4Command)
    {
       ...
       i4RetValue = read_otp_data((u16)(ptempbuf->u4Offset), ptempbuf->u4Length, pWorkingBuff);
