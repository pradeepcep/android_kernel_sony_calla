...
#define GPIO_bls_pin         (GPIO27 | 0x80000000)
unsigned int Cust_SetBacklight(int level)
{
    //mt_set_gpio_mode(GPIO_bls_pin,0);
	//mt_set_gpio_dir(GPIO_bls_pin, GPIO_DIR_OUT);

	//mt_set_gpio_out

	mt_set_gpio_mode(GPIO_bls_pin, GPIO_CTP_RST_PIN_M_GPIO);
  	mt_set_gpio_dir(GPIO_bls_pin, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_bls_pin, 1);
...
static struct cust_mt65xx_led cust_led_list[MT65XX_LED_TYPE_TOTAL] = {
	...
	//{"lcd-backlight",     MT65XX_LED_MODE_GPIO, (int)Cust_SetBacklight,{0}}, //Taylor-20140806
