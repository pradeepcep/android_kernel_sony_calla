 ...
 */
// 20150205 Wilson Close AIS Begin
#define TUNING_FOR_AIS  0 //1
// 20150205 Wilson Close AIS End
...
EIndex_CCM_T eIdx_CCM_Global = eIDX_CCM_D65;
...
{
	float CCM_RATIO = 1.0;
	float CCM_RATIO_MIN = 0.5; //0.8;//0.9;
	MINT32 HIGH_LV = 90;//30;
  MINT32 LOW_LV = 1;
  char debugFlag[PROPERTY_VALUE_MAX] = {'\0'};	
	MINT32 CCM22 =  Complement2((rCCM.cnv_6.val & 0x1FFF),13);
    #if 0
    MY_LOG("%s()\n", __FUNCTION__);
    // TODO: Add your code below...
    ...
    {
if(0&&(rCamInfo.rFlashInfo.isFlash == 0)) // Flash off
				{
					if(rCamInfo.rAEInfo.i4LightValue_x10 < HIGH_LV)
					{
						if(rCamInfo.rAEInfo.i4LightValue_x10 > LOW_LV )
							CCM_RATIO = 1.0 * ( HIGH_LV - rCamInfo.rAEInfo.i4LightValue_x10) / (float)(HIGH_LV-LOW_LV); 
						else
							CCM_RATIO = 0.8;//0.8;//0.9;
							
						if(CCM_RATIO < CCM_RATIO_MIN)
							CCM_RATIO = 0.8;//0.8;//0.9;
						if(CCM_RATIO > 1.0)
							CCM_RATIO = 0.8;//0.8;//0.9;
		...
		MFLOAT temp_01 = CCM_RATIO * (MFLOAT)digit_01;
		MFLOAT temp_02 = CCM_RATIO * (MFLOAT)digit_02;
		MFLOAT temp_10 = CCM_RATIO * (MFLOAT)digit_10;
		MFLOAT temp_12 = CCM_RATIO * (MFLOAT)digit_12;
		MFLOAT temp_20 = CCM_RATIO * (MFLOAT)digit_20;
		MFLOAT temp_21 = CCM_RATIO * (MFLOAT)digit_21;
        
        MINT32 final_00 = ((MFLOAT)digit_00-512.0) * CCM_RATIO + 0.5 + 512.0;
        MINT32 final_01 = (temp_01 > 0) ? temp_01 + 0.5 : temp_01 - 0.5;
        MINT32 final_02 = (temp_02 > 0) ? temp_02 + 0.5 : temp_02 - 0.5;
        MINT32 final_10 = (temp_10 > 0) ? temp_10 + 0.5 : temp_10 - 0.5;
        MINT32 final_11 = ((MFLOAT)digit_11-512.0) * CCM_RATIO + 0.5 + 512.0;
        MINT32 final_12 = (temp_12 > 0) ? temp_12 + 0.5 : temp_12 - 0.5;
        MINT32 final_20 = (temp_20 > 0) ? temp_20 + 0.5 : temp_20 - 0.5;
        MINT32 final_21 = (temp_21 > 0) ? temp_21 + 0.5 : temp_21 - 0.5;
        MINT32 final_22 = ((MFLOAT)digit_22-512.0) * CCM_RATIO + 0.5 + 512.0;
...
refine_ANR(RAWIspCamInfo const& rCamInfo, IspNvramRegMgr & rIspRegMgr, ISP_NVRAM_ANR_T& rANR)
{
    ...
    if(TUNING_FOR_AIS) {
        // enable sl2b for ais stage3/4, else diable it
        rANR.con1.bits.ANR_LCE_LINK = 0;
        if(IS_AIS) {
...
//                    THA     THB            THC     THD
    ...
    if  ( rCamInfo.eIdx_PCA_LUT != eIdx_PCA_LUT_new )
    {
		...
		if(eIdx_CCM_Global == eIDX_CCM_A || eIdx_CCM_Global == eIDX_CCM_TL84)
		{
...
// 20150205 Wilson Tuning Shading Ratio Begin
if (getSensorDev() == ESensorDev_Sub) { // main
    static const MINT32 rto[10] =
    {32, 28, 24, 19, 13, 6, 6, 6, 6, 6}; //Tower modify for iso1600 Noise 2014-12-26 
    //{6, 6, 6, 6, 6, 6, 6, 6, 6, 6};
    ...
    static const MINT32 rto[10] =
    {32, 32, 32, 30, 28, 25, 22, 18, 14, 10}; //Tower modify for iso1600 Noise 2014-12-26 



// 20150205 Wilson Tuning Shading Ratio End
