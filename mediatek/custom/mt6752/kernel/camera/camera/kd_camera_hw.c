		...
		{SENSOR_DRVNAME_OV5670_MIPI_RAW,
			{
				...
				{PDN,	Vol_High, 1},
				{RST,	Vol_Low,  0},
				{RST,	Vol_High, 0} //VCM PD GPIO75
			},
		},
		// Second_OV5670	//[Compal] Add it for Second sensor
		{SENSOR_DRVNAME_OV5670_CHICONY_MIPI_RAW,
			{
				{SensorMCLK,Vol_High, 0},
				{AVDD,	Vol_2800, 0},
				{DOVDD, Vol_1800, 0},
...
//				{RST,	Vol_High, 0}
		...
		// Sub_OV2680_CHICONY	//[Compal] Add it for Second sensor
		{SENSOR_DRVNAME_OV2680_CHICONY_MIPI_RAW,
			{
				{SensorMCLK,Vol_High, 0},
				{SUBVDD,Vol_1800, 0},
				{AVDD,	Vol_2800, 0},
...
//				{RST,	Vol_High, 0}
		...
		// Sub_OV2680 for PVT(HW modify schematic for open two camera issue)
		{SENSOR_DRVNAME_OV2680_MIPI_RAW_PVT,
			{
				{SensorMCLK,Vol_High, 0},
				{DOVDD, Vol_1800, 0},
				{SUBVDD,Vol_2800, 0},
...
BOOL hwpowerdown(PowerInformation pwInfo, char* mode_name)
{
	...
	// CEI comments start //
	//Add for sub cam vgp3 power
	else if(pwInfo.PowerType == SUBVDD)
	{
		if(PowerCustList.PowerCustInfo[4].Gpio_Pin == GPIO_UNSUPPORTED)
		{
			if(TRUE != hwPowerDown(pwInfo.PowerType,mode_name))
       	 	{
            		PK_DBG("[CAMERA SENSOR] Fail to enable vgp3 power\n");
            		//return FALSE;
            		ret = FALSE;
        	}
		}
		else{
			if(mt_set_gpio_mode(PowerCustList.PowerCustInfo[4].Gpio_Pin,PowerCustList.PowerCustInfo[4].Gpio_Mode)){PK_DBG("[CAMERA LENS] set gpio mode failed!! \n");}
			if(mt_set_gpio_dir(PowerCustList.PowerCustInfo[4].Gpio_Pin,GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! \n");}						
			if(mt_set_gpio_out(PowerCustList.PowerCustInfo[4].Gpio_Pin,PowerCustList.PowerCustInfo[4].Voltage)){PK_DBG("[CAMERA LENS] set gpio failed!! \n");}
			}
	}
	// CEI comments end //
...
int kdCISModulePowerOn(CAMERA_DUAL_CAMERA_SENSOR_ENUM SensorIdx, char *currSensorName, BOOL On, char* mode_name)
{
	...
	// CEI comments start //
	cam_hwid = board_type_with_hw_id();
	PK_DBG("camhw_id=%d\n",cam_hwid);
	if(strcmp(currSensorName,"ov2680mipiraw") == 0 )
	{
		if((cam_hwid == 0) || (cam_hwid == 1) || (cam_hwid == 4))
		{
			currSensorName = "ov2680mipiraw";
		}
		else
		{
			currSensorName = "ov2680mipirawpvt";
		}
	}
	else if(strcmp(currSensorName,"ov2680chiconymipiraw") == 0 )	//[Compal] Add it for Second sensor
	{
		if((cam_hwid == 0) || (cam_hwid == 1) || (cam_hwid == 4))
		{
			currSensorName = "ov2680chiconymipiraw";
		}
		else
		{
			currSensorName = "ov2680mipirawpvt";
    ...
    if (On) {
		...
		// CEI comments start //
		/*temporary solution*/
		if((strcmp(currSensorName,"ov5670mipi") == 0 )||(strcmp(currSensorName,"ov5670chiconymipi") == 0 ))//[Compal] Add it for Second sensor
       		pinSet[0][0] = (GPIO75 | 0x80000000);

		else if(strcmp(currSensorName,"imx134mipiraw") == 0 )
			pinSet[0][0] = (GPIO117 | 0x80000000);

		PK_DBG("[Vince Debug] (pinSet[0][0]:%u)(CAMERA_CMRST_PIN:%u)",pinSet[0][0],CAMERA_CMRST_PIN); //Remove it
		// CEI comments end //
