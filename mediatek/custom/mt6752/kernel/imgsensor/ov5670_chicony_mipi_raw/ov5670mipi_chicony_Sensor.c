...
#include "ov5670mipi_chicony_Sensor.h"			//[Compal] Add it for Second sensor

#define PFX "ov5670_chicony_camera_sensor"
//#define LOG_WRN(format, args...) xlog_printk(ANDROID_LOG_WARN ,PFX, "[%S] " format, __FUNCTION__, ##args)
//#defineLOG_INF(format, args...) xlog_printk(ANDROID_LOG_INFO ,PFX, "[%s] " format, __FUNCTION__, ##args)
//#define LOG_DBG(format, args...) xlog_printk(ANDROID_LOG_DEBUG ,PFX, "[%S] " format, __FUNCTION__, ##args)
#define LOG_INF(format, args...)	xlog_printk(ANDROID_LOG_INFO   , PFX, "[%s] " format, __FUNCTION__, ##args)

#define OTP_COMBINATION							//[Compal] Add it for Second sensor
static kal_uint16 ov5670ChiconyMID = 0x16;		//[Compal] Add it for Second sensor 

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static otp_struct otp_value={0,0,0};			//[Compal] Add it for Second sensor


static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5670_CHICONY_MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	
	.checksum_value = 0x523c51f6,//0x5785ca55,		//checksum value for Camera Auto Test
...
{
	int otp_flag,addr,temp,i,temp_test;
    ...
    write_cmos_sensor(0X3d8B,0x31);
    write_cmos_sensor(0X3d81,0x01);//load otp into buffer
    msleep(5);

/*
	for (i=0x7010; i<=0x7031; i++)
   {
       temp_test = 0;
       temp_test =read_cmos_sensor(i);
       LOG_INF("[CAM_CAL][read_otp_data] Address 0x%x data value is 0x%x\n", i, temp_test);
   }
 */
...
/*	//OTP into
	...
	if(addr !=0){
		...
		//OTP WB Calibration
        otp_flag=read_cmos_sensor(0x7010);
                LOG_INF("[CAM_CAL][read_otp_data] Address 0x7010 data value is 0x%x\n", otp_flag);
		addr=0;
		if((otp_flag & 0x0c)==0x04){
			addr=0x7027;//base address of WB Calibration group 3
			}
		else if((otp_flag & 0x30)==0x10){
			addr=0x701c;//base address of WB Calibration group 2
			}
		else if((otp_flag & 0xc0)==0x40){
	 		addr=0x7011;//base address of WB Calibration group 1
			}
                LOG_INF("[CAM_CAL][read_otp_data] addr value is 0x%x\n", addr);

		if(addr !=0){

			(*otp_ptr).flag = 0x40;
            temp=read_cmos_sensor(addr+AWB_LSB_OFFSET);
            (*otp_ptr).rg_ratio=(read_cmos_sensor(addr+AWB_RG_MSB_OFFSET)<<2)+((temp>>2) & 0x03);
            (*otp_ptr).bg_ratio=(read_cmos_sensor(addr+AWB_BG_MSB_OFFSET)<<2)+(temp & 0x03);
			(*otp_ptr).module_integrator_id=read_cmos_sensor(addr+MID_OFFSET);							//[Compal] Add it for Second sensor				
				LOG_INF("module_integrator_id: 0x%x\n", (*otp_ptr).module_integrator_id);				//[Compal] Add it for Second sensor
			}
		else{
			(*otp_ptr).rg_ratio=0;
			(*otp_ptr).bg_ratio=0;
			}
       LOG_INF("[CAM_CAL][read_otp_data] rg_ratio: 0x%x, bg_ratio: 0x%x\n", (*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio);
...
//	if ((*otp_ptr).flag & 0x40){
		...
		G_gain = 0x400 * G_gain / (Base_gain);
                ...
                LOG_INF("[CAM_CAL][read_otp_data] R_gain: 0x%x, B_gain: 0x%x, G_gain: 0x%x\n", R_gain, B_gain, G_gain);
...
static void update_otp(void)
{
	//otp_struct otp_value={0,0,0}; //[Compal] Add it for Second sensor
	int return_value;

	return_value = read_otp(&otp_value);
	LOG_INF("OTP return value = %d\n", return_value);

	if ( return_value != 0x40  )
	{
		LOG_INF("Update OTP return without apply_otp() !!\n");
		return;
	}
...
{
	LOG_INF("OV5670_chicony_Sensor_Init_2lane E\n");
...
{
	LOG_INF(" OV5670_chicony PreviewSetting_2lane enter\n");
...
{
	LOG_INF("OV5670_chicony CaptureSetting_2lane enter! currefps:%d\n",currefps);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	int return_otp_value;		//[Compal] Add it for Second sensor
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			*sensor_id+=1;		//[Compal] Add it for Second sensor
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
	  			//return ERROR_NONE;        		//[Compal] Add it for Second sensor
	  			break;                      		//[Compal] Add it for Second sensor
			}
		...
		if (*sensor_id == imgsensor_info.sensor_id) {			//[Compal] Add it for Second sensor
			LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			break;
		}	                                               	//[Compal] Add it for Second sensor

		retry = 2;
	}
	if (*sensor_id != imgsensor_info.sensor_id) {
		// if Sensor ID is not correct, Must set *sensor_id to 0xFFFFFFFF
		LOG_INF("After Read sensor id , id: 0x%x -- Second sensor id: 0x%x\n", *sensor_id,imgsensor_info.sensor_id);
	...
	sensor_init();
										//[Compal] Add it for Second sensor
	return_otp_value = read_otp(&otp_value);
	LOG_INF("OTP return value = 0x%x\n", return_otp_value);
	if ( return_otp_value == 0x40  )
	{
		if (!(otp_value.module_integrator_id == ov5670ChiconyMID))
		{
			LOG_INF("OV5670 module ID: 0x%x\n", otp_value.module_integrator_id);
			LOG_INF("It is OV5670 main source module.");
			*sensor_id = 0xFFFFFFFF;
			return ERROR_SENSOR_CONNECT_FAIL;
		}
	}else{
		LOG_INF("Write AWB OTP Fail \n");
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}
#endif                                  		//[Compal] Add it for Second sensor
...
static kal_uint32 open(void)
{
	...
	int return_otp_value;		
	LOG_INF("PLATFORM:OV5670_chicony,MIPI 2LANE\n");
	LOG_INF("preview 1280*960@30fps,864Mbps/lane; video 1280*960@30fps,864Mbps/lane; capture 5M@30fps,864Mbps/lane\n");

get_imgsensor_id((UINT32 *)(&sensor_id));		//[Compal] Add it for Second sensor
#if 0		//[Compal] Add it for Second sensor
	//sensor have two i2c address 0x6c 0x6d & 0x21 0x20, we should detect the module used i2c address
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		...
		do {
			...
			LOG_INF("Read sensor id fail, Second source id: 0x%x, Read sensor id from I2C: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
...
UINT32 OV5670_CHICONY_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	OV5670_CHICONY_MIPI_RAW_SensorInit	*/
