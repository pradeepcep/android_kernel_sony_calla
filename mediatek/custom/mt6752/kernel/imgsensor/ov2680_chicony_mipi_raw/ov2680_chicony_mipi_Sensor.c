...
#include "ov2680_chicony_mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "OV2680_chicony_camera_sensor"
#define LOG_1 LOG_INF("OV2680_CHICONY,MIPI 1LANE\n")
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV2680_CHICONY_MIPI_SENSOR_ID,  /*sensor_id = 0x2681*/ //record sensor id defined in Kd_imgsensor.h
...
/*according toov2680 chicony datasheet p53 image cropping*/
...
static void set_dummy()
{
	...
	/*  Add dummy pixels: */
    /* 0x380c [0:4], 0x380d defines the PCLKs in one line of OV2680 CHICONY  */  
    /* Add dummy lines:*/
    /* 0x380e [0:1], 0x380f defines total lines in one frame of OV2680 CHICONY */
...
static void sensor_init(void)
{
   ...
   */
	...
	write_cmos_sensor(0x3086, 0x00); //01->00
	write_cmos_sensor(0x3501, 0x4e); //26->4e
	write_cmos_sensor(0x3502, 0xe0); //40->e0
	...
	write_cmos_sensor(0x3620, 0x24); //26->24
	write_cmos_sensor(0x3621, 0x34); //37->34
	write_cmos_sensor(0x3622, 0x03); //04->03
	write_cmos_sensor(0x3628, 0x00);
	write_cmos_sensor(0x3701, 0x64);
	write_cmos_sensor(0x3705, 0x3c);
	write_cmos_sensor(0x370c, 0x50);
	write_cmos_sensor(0x370d, 0xc0);
	write_cmos_sensor(0x3718, 0x80); //88->80
	write_cmos_sensor(0x3720, 0x00);
	write_cmos_sensor(0x3721, 0x09); //00->09
	write_cmos_sensor(0x3722, 0x06); //00->06
	write_cmos_sensor(0x3723, 0x59); //00->59
	write_cmos_sensor(0x3738, 0x00); //00->99
	write_cmos_sensor(0x370a, 0x23); //23->21
	...
	write_cmos_sensor(0x3806, 0x04);
	write_cmos_sensor(0x3807, 0xbf);
	write_cmos_sensor(0x3808, 0x06); //03->06
	write_cmos_sensor(0x3809, 0x40); //20->40
	write_cmos_sensor(0x380a, 0x04); //02->04
	write_cmos_sensor(0x380b, 0xb0); //58->b0
	write_cmos_sensor(0x380c, 0x06);
	write_cmos_sensor(0x380d, 0xa4); //ac->a4
	write_cmos_sensor(0x380e, 0x05); //02->05
	write_cmos_sensor(0x380f, 0x0e); //84->0e
	write_cmos_sensor(0x3810, 0x00);
	write_cmos_sensor(0x3811, 0x08); //04->08
	write_cmos_sensor(0x3812, 0x00);
	write_cmos_sensor(0x3813, 0x08); //04->08
	write_cmos_sensor(0x3814, 0x11); //31->11
	write_cmos_sensor(0x3815, 0x11); //31->11
	write_cmos_sensor(0x3819, 0x04);
	write_cmos_sensor(0x3820, 0xc0); //c2->c0
	write_cmos_sensor(0x3821, 0x00); //01->00
	write_cmos_sensor(0x4000, 0x81);
	write_cmos_sensor(0x4001, 0x40);
	write_cmos_sensor(0x4008, 0x02); //00->02
	write_cmos_sensor(0x4009, 0x09); //03->09
	write_cmos_sensor(0x4602, 0x02);
	write_cmos_sensor(0x481f, 0x36);
	write_cmos_sensor(0x4825, 0x36);
	write_cmos_sensor(0x4837, 0x18); //30->18
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	LOG_INF("roger_Read sensor id , id: 0x%x -- Second sensor id: 0x%x\n", *sensor_id,imgsensor_info.sensor_id);

	mt_set_gpio_mode(CAMERA_CMSOURCE_PIN, CAMERA_CMSOURCE_PIN_M_GPIO);	//[Compal] Add it for Second sensor
	mt_set_gpio_dir(CAMERA_CMSOURCE_PIN, CAMERA_CMSOURCE_PIN_M_GPIO);	// GPIO_DIR_IN or GPIO_DIR_OUT
	if (mt_get_gpio_in(CAMERA_CMSOURCE_PIN) != 0)
	{
		// if Sensor gpio is not correct, Must set *sensor_id to 0xFFFFFFFF
		LOG_INF("ov2680 F_CAM_ID = 1, it is ov2680 main source");
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}									//[Compal] Add it for Second sensor
	LOG_INF("roger_ov2680_chicony_CAMERA_CMSOURCE_PIN_OK");
	return ERROR_NONE;
}
...
/*UINT32 OV2680_CHICONY_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)*/
UINT32 OV2680CHICONYMIPISensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)		

{
	...
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	OV2680_CHICONY_MIPI_RAW_SensorInit	*/
