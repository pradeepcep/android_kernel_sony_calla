...
static kal_uint16 ov5670LiteonMID = 0x15;	//[Compal] Add it for Second sensor
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		...
		LOG_INF("After Read sensor id , id: 0x%x -- main sensor id: 0x%x\n", *sensor_id,imgsensor_info.sensor_id);
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}

#ifdef OTP_COMBINATION										//[Compal] Add it for Second sensor
//#if 0
	...
	{
		if (!(otp_value.module_integrator_id == ov5670LiteonMID))
		{
			LOG_INF("OV5670 module ID: 0x%x\n", otp_value.module_integrator_id);
			LOG_INF("It is OV5670 second source module.");
			*sensor_id = 0xFFFFFFFF;
			return ERROR_SENSOR_CONNECT_FAIL;
		}
	}else{
...
static kal_uint32 open(void)
{
	...
	int return_otp_value;			//[Compal] Add it for Second sensor
	LOG_INF("PLATFORM:OV5670,MIPI 2LANE\n");
...
get_imgsensor_id((UINT32 *)(&sensor_id));		//[Compal] Add it for Second sensor
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			LOG_INF("Read sensor id fail, Main source id: 0x%x, Read sensor id from I2C: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
