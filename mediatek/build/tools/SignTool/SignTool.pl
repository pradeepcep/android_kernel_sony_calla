...
my $key = "$cfg_dir/IMG_AUTH_KEY_TEST.ini";
...
foreach my $img (@imgs_need_sign) {
	...
	system("$command $key $signed_cfg $dir/$img $dir/signed_bin/$signed_img $nand_page_size") == 0 or die "sign image fail";
}
...
foreach my $img (@imgs_need_sign) {
	if (-e "$dir/$img") {		
		my $signed_cfg = "$cfg_dir/$img.ini";
        	...
        	print "Sign Image '$dir/$img' with cfg '$signed_cfg'...\n";
		system("$command $key $signed_cfg $dir/$img $dir/signed_bin/${img}-sign $nand_page_size") == 0 or die "sign EMMC image fail";
	}
}
...
foreach my $img (@imgs_need_sign) {
	if (-e "$dir/$img") {		
		...
		$signed_img =~ s/\./-sign\./;
        	...
        	print "Sign Image '$dir/$img' with cfg '$signed_cfg'...\n";
		system("$command $key $signed_cfg $dir/$img $dir/signed_bin/$signed_img $nand_page_size") == 0 or die "sign EMMC image fail";
	}
}
	...
	foreach my $img (@imgs_need_sign) {
		if (-e "$dir/$img") {		
			...
			$signed_img =~ s/\./-sign\./;
        		...
        		print "Sign Image '$dir/$img' with cfg '$signed_cfg'...\n";
			system("$command $key $signed_cfg $dir/$img $dir/signed_bin/$signed_img $nand_page_size") == 0 or die "sign SECRO image fail";
		}
	}
}
