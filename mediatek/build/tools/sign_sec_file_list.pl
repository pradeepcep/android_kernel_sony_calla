...
if (-e "$and_secfl")
{
	...
	PrintDependency("$key_dir/IMG_AUTH_KEY_TEST.ini");
	PrintDependency("$cfg_dir/SFL_CFG.ini");
	PrintDependency($and_secfl);
	system("./$sign_tool $key_dir/IMG_AUTH_KEY_TEST.ini $cfg_dir/SFL_CFG.ini $and_secfl $s_andro_fl");
...
{				
	...
	system("./$sign_tool $key_dir/IMG_AUTH_KEY_TEST.ini $cfg_dir/SFL_CFG.ini $secro_secfl $s_secro_fl_o1");
