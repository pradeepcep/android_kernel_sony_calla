...
KERNEL_PLATFORM_CONFIG_FILE := $(MTK_ROOT_CONFIG)/$(ARCH_MTK_PLATFORM)/autoconfig/kconfig/platform
#[LY28] <== CCI KLog, added by Jimmy@CCI
# *************************************************************************
# Set PHONY
# *************************************************************************
...
BOOT_AID ?= 1
S1_RELEASE_ROOT := vendor/semc/s1_release
S1SBL_IMG := $(S1_RELEASE_ROOT)/S1_img_delivery/S1SBL/S1SBL_AID_$(BOOT_AID).bin
S1SBL_SHA := sha256
LOADER_PAIR_TOOL := $(S1_RELEASE_ROOT)/tools/loaderpairing/pair_loaders.pl
endif
...
ifeq ($(BUILD_PRELOADER),yes)
  ...
  ifeq ($(SONY_S1_SUPPORT),yes)
	$(hide) perl $(LOADER_PAIR_TOOL) -$(S1SBL_SHA) $(S1SBL_IMG) mediatek/platform/`echo $(MTK_PLATFORM) | tr A-Z a-z`/preloader/src/s1/inc/s1sbl_hash.h >/dev/null
  endif
	$(hide) cd $(PRELOADER_WD) && \
	  ...
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG))
  ifeq ($(SONY_S1_SUPPORT),yes)
	$(hide) cat $(PRELOADER_IMAGES) $(S1SBL_IMG) > $(PRELOADER_IMAGES).cat && \
          ($(if $(strip $(ACTION)),:,$(call copytoout,$(PRELOADER_IMAGES).cat,$(LOGDIR)/$(PROJECT))) && \
          $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG) || \
          $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG))
    endif
  endif
...
#[LY28] ==> CCI KLog, added by Jimmy@CCI
	$(hide) sed -i '/CCI KLOG/d' $(KERNEL_PLATFORM_CONFIG_FILE)
	$(hide) sed -i '/CCI_KLOG/d' $(KERNEL_PLATFORM_CONFIG_FILE)
	$(hide) echo "# CCI KLOG" >> $(KERNEL_PLATFORM_CONFIG_FILE)
    ifeq ($(CCI_TARGET_KLOG),true)
		$(hide) echo "CONFIG_CCI_KLOG=y" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "CONFIG_CCI_KLOG_RECORD_KERNEL_TIMESTAMP=y" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_RECORD_RPM_VERSION is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "CONFIG_CCI_KLOG_START_ADDR_PHYSICAL="$(CCI_TARGET_KLOG_START_ADDR_PHYSICAL) >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "CONFIG_CCI_KLOG_SIZE="$(CCI_TARGET_KLOG_SIZE) >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "CONFIG_CCI_KLOG_HEADER_SIZE="$(CCI_TARGET_KLOG_HEADER_SIZE) >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "CONFIG_CCI_KLOG_CRASH_SIZE="$(CCI_TARGET_KLOG_CRASH_SIZE) >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "CONFIG_CCI_KLOG_APPSBL_SIZE="$(CCI_TARGET_KLOG_APPSBL_SIZE) >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "CONFIG_CCI_KLOG_KERNEL_SIZE="$(CCI_TARGET_KLOG_KERNEL_SIZE) >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "CONFIG_CCI_KLOG_ANDROID_MAIN_SIZE="$(CCI_TARGET_KLOG_ANDROID_MAIN_SIZE) >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "CONFIG_CCI_KLOG_ANDROID_SYSTEM_SIZE="$(CCI_TARGET_KLOG_ANDROID_SYSTEM_SIZE) >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "CONFIG_CCI_KLOG_ANDROID_RADIO_SIZE="$(CCI_TARGET_KLOG_ANDROID_RADIO_SIZE) >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "CONFIG_CCI_KLOG_ANDROID_EVENTS_SIZE="$(CCI_TARGET_KLOG_ANDROID_EVENTS_SIZE) >> $(KERNEL_PLATFORM_CONFIG_FILE)
      ifneq ($(TARGET_BUILD_VARIANT),user)
			$(hide) echo "CONFIG_CCI_KLOG_ALLOW_FORCE_PANIC=y" >> $(KERNEL_PLATFORM_CONFIG_FILE)
      else # ifneq ($(TARGET_BUILD_VARIANT),user)
			$(hide) echo "# CONFIG_CCI_KLOG_ALLOW_FORCE_PANIC is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
      endif # ifneq ($(TARGET_BUILD_VARIANT),user)
      ifeq ($(CCI_KLOG_SUPPORT_CCI_ENGMODE),1)
			$(hide) echo "CONFIG_CCI_KLOG_SUPPORT_CCI_ENGMODE=y" >> $(KERNEL_PLATFORM_CONFIG_FILE)
      else # ifeq ($(CCI_KLOG_SUPPORT_CCI_ENGMODE),1)
			$(hide) echo "# CONFIG_CCI_KLOG_SUPPORT_CCI_ENGMODE is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
      endif # ifeq ($(CCI_KLOG_SUPPORT_CCI_ENGMODE),1)
      ifeq ($(CCI_KLOG_SUPPORT_RESTORATION),1)
			$(hide) echo "CONFIG_CCI_KLOG_SUPPORT_RESTORATION=y" >> $(KERNEL_PLATFORM_CONFIG_FILE)
      else # ifeq ($(CCI_KLOG_SUPPORT_RESTORATION),1)
			$(hide) echo "# CONFIG_CCI_KLOG_SUPPORT_RESTORATION is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
      endif # ifeq ($(CCI_KLOG_SUPPORT_RESTORATION),1)
    else # ifeq ($(CCI_TARGET_KLOG),true)
		$(hide) echo "# CONFIG_CCI_KLOG is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_RECORD_KERNEL_TIMESTAMP is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_RECORD_RPM_VERSION is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_START_ADDR_PHYSICAL is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_SIZE is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_HEADER_SIZE is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_CRASH_SIZE is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_APPSBL_SIZE is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_KERNEL_SIZE is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_ANDROID_MAIN_SIZE is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_ANDROID_SYSTEM_SIZE is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_ANDROID_RADIO_SIZE is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_ANDROID_EVENTS_SIZE is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_ALLOW_FORCE_PANIC is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_SUPPORT_CCI_ENGMODE is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
		$(hide) echo "# CONFIG_CCI_KLOG_SUPPORT_RESTORATION is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
...
# Add for handle oemS and oemF by adb reboot command
	$(hide) sed -i '/SONY_S1_SUPPORT/d' $(KERNEL_PLATFORM_CONFIG_FILE)
    ifeq ($(SONY_S1_SUPPORT),yes)
	$(hide) echo "CONFIG_SONY_S1_SUPPORT=y" >> $(KERNEL_PLATFORM_CONFIG_FILE)
    else # ifeq ($(SONY_S1_SUPPORT),yes)
	$(hide) echo "# CONFIG_SONY_S1_SUPPORT is not set" >> $(KERNEL_PLATFORM_CONFIG_FILE)
    endif # ifeq ($(SONY_S1_SUPPORT),yes)
# CEI comments end
...
# enable EMAKE for build Android
  ifneq ($(EMAKE),yes)
	...
	$(hide) ($(MAKECMD) $(SOMC_FULLBUILD_TARGET) $(DEAL_STDOUT);exit $${PIPESTATUS[0]} && \
        $(if $(filter clean,$(ACTION)),,$(call chkImgSize,$(ACTION),$(PROJECT),$(SCATTER_FILE),$(if $(strip $(ACTION)),$(CHECK_IMAGE),$(ANDROID_IMAGES)),$(DEAL_STDOUT),&&)) \
          $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG) $(ACTION) || \
          $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG) $(ACTION))
    endif
	...
	$(hide) ($(EMAKECMD) $(SOMC_FULLBUILD_TARGET) $(DEAL_STDOUT);exit $${PIPESTATUS[0]})
	$(hide) $(if $(filter clean,$(ACTION)),,$(call chkImgSize,$(ACTION),$(PROJECT),$(SCATTER_FILE),$(if $(strip $(ACTION)),$(CHECK_IMAGE),$(ANDROID_IMAGES)),$(DEAL_STDOUT),&&)) \
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG) $(ACTION) || \
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG) $(ACTION)
    endif
