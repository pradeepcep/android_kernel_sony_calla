...
//CEI comments start//
//YF-index_0 for 100Kohm, index_1 for 150Kohm, index_2 for 220Kohm, index_3 for 330Kohm, index_4 for 680Kohm, index_5 for others
kal_int32 g_battery_id_voltage[]={1060, 1210, 1330, 1430, 1550, -1};
//CEI comments end//

#ifdef MTK_GET_BATTERY_ID_BY_AUXADC
void fgauge_get_profile_id(void) //YF
{
	int id_volt = 0;
    int id = 0;
	int ret = 0;
...
//CEI comments start//
extern unsigned char isInternalCharger;
//CEI comments end//
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
int BattThermistorConverTemp(int Res) //return Tbat in degC based on Res in ohm
{
    int i=0;
    int RES1=0,RES2=0;
...
//CEI comments start//
	...
	if(isInternalCharger)
		batt_temperature_table = &Batt_Temperature_Table[g_fg_battery_id][0];
	else
		batt_temperature_table = &BQ24158_Batt_Temperature_Table[0];
//CEI comments end//
...
//CEI comments start//
	...
	if(isInternalCharger)
		batt_temperature_table = &Batt_Temperature_Table[0];
	else
		batt_temperature_table = &BQ24158_Batt_Temperature_Table[0];
//CEI comments end//
...
int BattVoltToTemp(int dwVolt) //return Tbat in degC based on dwVolt in mV
{
    kal_int64 TRes_temp;
	kal_int64 TRes;
    int sBaTTMP = -100;
...
//CEI comments start//
	...
	TRes_temp = (24000*(kal_int64)dwVolt); 
//CEI comments end//
    ...
    /* convert resister to temperature */
    sBaTTMP = BattThermistorConverTemp((int)TRes);
      
    return sBaTTMP;
}

int force_get_tbat(kal_bool update) //return Tbat in degC, if update is true->return updated Tbat, is false->return previous Tbat
{
...
void fgauge_construct_r_table_profile(kal_int32 temperature, R_PROFILE_STRUC_P temp_profile_p)
{
    ...
    kal_int32 temp_r_1 = 0, temp_r_2 = 0;
        ...
        bm_print(BM_LOG_CRTI, "[fgauge_construct_r_table_profile] temperature=%d\r\n", temperature);
...
void fgauge_construct_table_by_temp(void)
{
    ...
    {
        bm_print(BM_LOG_CRTI, "[fgauge_construct_table_by_temp] reconstruct table by temperature change from (%d) to (%d) curr_temp=%d\r\n", last_temp, avg_temp, curr_temp);
...
//CEI comments start//
    if(isInternalCharger)
        ...
        charging_current = BQ24158_get_charging_setting_current();    
//CEI comments end//
...
kal_int32 get_dynamic_period(int first_use, int first_wakeup_time, int battery_capacity_level) //return wait up period(in sec) based on various Vbat & Tbat
//When Vbat > 3.6V -> wait up period = ((gFG_BATT_CAPACITY*battery_capacity_level*3600)/100)/I_sleep
//When 3.5V < Vbat <= 3.6V -> wait up period = 300 secs
//When Vbat <= 3.5V -> wait up period = 30 secs
{
//modify by willcai 2014-5-29 begin
...
#endif                                                         
kal_int32 battery_meter_get_battery_voltage(kal_bool update) //get BATSNS on MT6352
{
    int ret=0;
    int val=5;
...
//CEI comments start//
	if(isInternalCharger)
	{
	bm_print(BM_LOG_CRTI, "[battery_meter_get_charging_current_imm] Using internal charger IC & Rsense 56mohm\n");
	 ICharging = (ADC_I_SENSE - ADC_BAT_SENSE + g_I_SENSE_offset)*1000/CUST_R_SENSE;
	}
	else
	{
	bm_print(BM_LOG_CRTI, "[battery_meter_get_charging_current_imm] Using BQ24158 charger IC & Rsense 68mohm\n");
	ICharging = (ADC_I_SENSE - ADC_BAT_SENSE + g_I_SENSE_offset)*1000/68;
	}
//CEI comments end//
...
kal_int32 battery_meter_get_charging_current(void) //return charging Ibat, in mA
{
//modify by willcai 2014-5-29 begin
...
//CEI comments start//
	if(isInternalCharger)
	{
	bm_print(BM_LOG_CRTI, "[battery_meter_get_charging_current] Using internal charger IC & Rsense 56mohm\n");
        ICharging = (ADC_I_SENSE - ADC_BAT_SENSE + g_I_SENSE_offset)*1000/CUST_R_SENSE;
	}
	else
	{
	bm_print(BM_LOG_CRTI, "[battery_meter_get_charging_current] Using BQ24158 charger IC & Rsense 68mohm\n");
	ICharging = (ADC_I_SENSE - ADC_BAT_SENSE + g_I_SENSE_offset)*1000/68;
	}
//CEI comments end//
    }
    ...
    bm_print(BM_LOG_CRTI, "[g_Get_I_Charging] ICharging=%d isInternalCharger=%d g_I_SENSE_offset=%d CUST_R_SENSE=%d\r\n", ICharging, isInternalCharger, g_I_SENSE_offset, CUST_R_SENSE);
    return ICharging;	
#else
    return 0;
#endif	
...
kal_int32 battery_meter_get_battery_temperature(void) //return Tbat in degC
{
#ifdef MTK_BATTERY_LIFETIME_DATA_SUPPORT    
...
kal_int32 battery_meter_get_charger_voltage(void) //return Vcharger, in mV
{
    int ret=0;
    int val=0;
...
kal_int32 battery_meter_sync(kal_int32 bat_i_sense_offset) //set g_I_SENSE_offset as bat_i_sense_offset, always return 0
{
#if defined(CONFIG_POWER_EXT)
    return 0;
#else
...
kal_int32 battery_meter_get_battery_zcv(void) //return zcv of battery, in mV
{
#if defined(CONFIG_POWER_EXT)
    return 3987;
#else
...
kal_int32 battery_meter_get_tempR(kal_int32 dwVolt) //return Rtbat, in ohm
{
#if defined(CONFIG_POWER_EXT)
    return 0;
#else
...
//CEI comments start//
    ...
    TRes = (24000*dwVolt) / (RBAT_PULL_UP_VOLT-dwVolt);        
//CEI comments end//
...
kal_int32 battery_meter_get_tempV(void) //return Vtbat, in mV
{
#if defined(CONFIG_POWER_EXT)
	return 0;
#else
...
kal_int32 battery_meter_get_VSense(void) //get ISENSE on MT6352
{
#if defined(CONFIG_POWER_EXT)
	return 0;
#else
