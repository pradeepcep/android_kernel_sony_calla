...
unsigned int g_BatteryNotifyCode=0x0000; //0x0001->VCHARGER, 0x0002->VBATTEMP, 0x0004->ICHARGING, 0x0008->VBAT, 0x0010->CHARGINGTIME
unsigned int g_BN_TestMode=0x0000; //0x0001->VCHARGER, 0x0002->VBATTEMP, 0x0003->ICHARGING, 0x0004->VBAT, 0x0005->CHARGINGTIME
...
static int g_wireless_state = 0; //always is 0
#endif
...
int read_tbat_value(void) //return Tbat in degC
{
    return BMT_status.temperature;
}

int get_charger_detect_status(void) //return charger if exist, 1->exist, others->don't exist
{
	kal_bool chr_status;
...
///////////////////////////////////////////////////////////////////////////////////////////
kal_bool upmu_is_chr_det(void) //return charger if exist & usb if isn't host, 1->charger exist & usb isn't host, 0->others
{
#if !defined(CONFIG_POWER_EXT)
	kal_uint32 tmp32;
#endif	
...
void update_charger_info(int wireless_state) //no body call the function
{
...
static void wireless_update(struct wireless_data *wireless_data) //YF
{
...
#if defined(MTK_TEMPERATURE_RECHARGE_SUPPORT)
PMU_STATUS do_batt_temp_state_machine(void) //return PMU_STATUS_OK->battery temperature is within charging range(g_batt_temp_status = TEMP_POS_NORMAL), others->battery temperature isn't within charging range(g_batt_temp_status != TEMP_POS_NORMAL)
{
...
void mt_battery_GetBatteryData(void) //get BMT_status.bat_vol, charger_vol, ICharging, SOC, temperature, temperatureR, temperatureV, UI_SOC, Vsense, ZCV into BMT_status
{ 
...
//if defined(MTK_JEITA_STANDARD_SUPPORT)->return PMU_STATUS_OK->means set CV after temperature changed, others->don't set CV
//if non-defined(MTK_JEITA_STANDARD_SUPPORT)->return PMU_STATUS_OK->battery temperature is within charging range(g_batt_temp_status = TEMP_POS_NORMAL), others->battery temperature isn't within charging range(g_batt_temp_status != TEMP_POS_NORMAL)
{	
...
//CEI comments start//
    ...
    {
	    if( BQ24158_do_jeita_state_machine() == PMU_STATUS_FAIL)
	    {
	        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] JEITA : fail\n");
	        status = PMU_STATUS_FAIL;
	    }
...
//if (BMT_status.charger_exist == KAL_TRUE)->return PMU_STATUS_OK->means charger voltage is ok, others->charger voltage is OV or UV
//if (BMT_status.charger_exist == KAL_FALSE)->always return PMU_STATUS_OK
{
	PMU_STATUS status = PMU_STATUS_OK;
    #if defined(MTK_DUAL_INPUT_CHARGER_SUPPORT)
...
//always return PMU_STATUS_OK if ( (g_battery_thermal_throttling_flag==2) || (g_battery_thermal_throttling_flag==3) )
//return PMU_STATUS_FAIL if (BMT_status.total_charging_time >= MAX_CHARGING_TIME), others->return PMU_STATUS_OK
{
    PMU_STATUS status = PMU_STATUS_OK;
    ...
    {    
        /* Charging OT */
//CEI comments start//
        if((((BMT_status.charger_type == STANDARD_CHARGER) || (BMT_status.charger_type == APPLE_2_1A_CHARGER) ) && BMT_status.total_charging_time >= MAX_CHARGING_TIME) || (BMT_status.total_charging_time >= PC_MAX_CHARGING_TIME))
//CEI comments end//
        {
...
//return PMU_STATUS_FAIL if ((g_call_state == CALL_ACTIVE) && (BMT_status.bat_vol > V_CC2TOPOFF_THRES)), others->return PMU_STATUS_OK
{
	PMU_STATUS status = PMU_STATUS_OK;
...
//01-set BMT_status.bat_charging_state as CHR_ERROR if ((cmd_discharging == 1) || (mt_battery_CheckBatteryTemp() != PMU_STATUS_OK) || 
//	(mt_battery_CheckChargerVoltage() != PMU_STATUS_OK) || (mt_battery_CheckChargingTime() != PMU_STATUS_OK))
//02-set BMT_status.bat_charging_state as CHR_HOLD if (mt_battery_CheckCallState() != PMU_STATUS_OK)
//03-set BMT_status.bat_charging_state as CHR_PRE if (cmd_discharging == 0)
{
	if (cmd_discharging == 1) {
...
static void mt_battery_notify_TotalChargingTime_check(void) //YF
{
...
static void mt_battery_notify_VBat_check(void) //YF
{
...
static void mt_battery_notify_ICharging_check(void) //YF
{
...
static void mt_battery_notify_VBatTemp_check(void) //YF
{
#if defined(BATTERY_NOTIFY_CASE_0002_VBATTEMP)

	if(BMT_status.temperature >= MAX_CHARGE_TEMPERATURE) //MAX_CHARGE_TEMPERATURE is 60 degC
    {
...
static void mt_battery_notify_VCharger_check(void) //YF
{
    ...
    #if !defined(MTK_DUAL_INPUT_CHARGER_SUPPORT)
    if(BMT_status.charger_vol > V_CHARGER_MAX) //V_CHARGER_MAX is 6500 mV
    #else
    if(BMT_status.charger_vol >= v_charger_max)
    #endif
    {
...
static void mt_battery_notify_UI_test(void) //YF
{
...
//if (enable batTT) & (no defined MTK_JEITA_STANDARD_SUPPORT) & (BMT_status.temperature >= 60) & (g_platform_boot_mode is not META_BOOT & ADVMETA_BOOT & ATE_FACTORY_BOOT)->shutdown system
{
...
CHARGER_TYPE mt_charger_type_detection(void) //return charger type and save into BMT_status.charger_type, refer to CHARGER_TYPE
{
...
static void mt_battery_charger_detect_check(void) //if usb cable pluged->wake_lock & setting BMT_status.charger_type & charger_exist & call mt_usb_connect()
//if usb cable unplugged->un_wake_lock & cleanning BMT_status.charger_type & charger_exist & call mt_usb_disconnect()
{
    if( upmu_is_chr_det() == KAL_TRUE )
    {
...
//CEI comments start//
	if(isInternalCharger)
	 ...
	 BQ24158_mt_battery_charging_algorithm();
//CEI comments end//
    }
...
{
//CEI comments start//
	battery_xlog_printk(BAT_LOG_CRTI, "[get_charging_control] isInternalCharger=%d\n", isInternalCharger);
	if(isInternalCharger)
	{
	battery_xlog_printk(BAT_LOG_CRTI, "[get_charging_control] Using internal charger IC\n");
	// Enable internal charger IC
	battery_charging_control = chr_control_interface;
	}
	else
	{
	battery_xlog_printk(BAT_LOG_CRTI, "[get_charging_control] Using BQ24158 charger IC\n");
	// Enable BQ24158 charger IC
...
//CEI comments start//
	...
	seq_printf(m, "=> get_usb_current_unlimited=%d, \ncmd_discharging = %d\n", BQ24158_get_usb_current_unlimited(), cmd_discharging);
//CEI comments end//
...
//CEI comments start//
	if(isInternalCharger)
        ...
        BQ24158_set_usb_current_unlimited(cmd_current_unlimited);
//CEI comments end//
        if (cmd_discharging == 1) {
