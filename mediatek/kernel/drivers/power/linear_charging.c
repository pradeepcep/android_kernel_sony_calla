...
#define TA_CHARGING_CURRENT			    CHARGE_CURRENT_1500_00_MA
 ...
 // ============================================================ //
//CEI comments start//
static kal_uint32 g_bcct_flag=0;
static CHR_CURRENT_ENUM g_temp_CC_value = CHARGE_CURRENT_0_00_MA; //stores ibat
static kal_uint32 g_usb_state = USB_UNCONFIGURED;
//CEI comments end//
 ...
 kal_bool ulc_cv_charging_current_flag = KAL_FALSE; //ulc_cv_charging_current_flag always is KAL_FALSE
...
//CEI comments start//
  ...
  static kal_bool temp_error_recovery_chr_flag =KAL_TRUE;
//set as KAL_FALSE if ((g_temp_status == TEMP_ABOVE_POS_60) ||(g_temp_status == TEMP_BELOW_NEG_10)) in BAT_BatteryStatusFailAction()
//set as KAL_TRUE if ((g_temp_status != TEMP_ABOVE_POS_60) ||(g_temp_status != TEMP_BELOW_NEG_10)) in BAT_BatteryStatusFailAction()
//CEI comments end//
#endif
...
kal_uint32 get_charging_setting_current() //return ibat
{
...
static BATTERY_VOLTAGE_ENUM select_jeita_cv(void) //return CV based on g_temp_status
{
...
//CEI comments start//
        battery_xlog_printk(BAT_LOG_CRTI, "select_jeita_cv() cv_voltage=%d  g_temp_status=%d\n\r", cv_voltage, g_temp_status);
//CEI comments end//

    return cv_voltage;
}

PMU_STATUS do_jeita_state_machine(void) //return PMU_STATUS_OK->means set CV after temperature changed, others->don't set CV
{
...
static void set_jeita_charging_current(void) //set g_temp_CC_value based on g_temp_status & BMT_status.charger_type
{
#ifdef CONFIG_USB_IF
	if(BMT_status.charger_type == STANDARD_HOST)
		return;
...
bool get_usb_current_unlimited(void) //YF
{
...
void set_usb_current_unlimited(bool enable) //set usb_unlimited from current_cmd_write()
{
...
kal_uint32 set_bat_charging_current_limit(int current_limit) //return 1 & set g_temp_CC_value if (current_limit != -1), return 0 if (current_limit == -1)
{
...
//CEI comments start//
static void select_charging_curret(void) //set g_temp_CC_value based on g_temp_status & BMT_status.charger_type
//CEI comments end//
{
...
//CEI comments end//
			#ifdef CONFIG_USB_IF
            {
                if (g_platform_boot_mode== KERNEL_POWER_OFF_CHARGING_BOOT || LOW_POWER_OFF_CHARGING_BOOT)
                {
                    g_temp_CC_value = USB_CHARGER_CURRENT_CONFIGURED;
                }
                else if (g_usb_state == USB_SUSPEND)
                {
...
//CEI comments end//
		...
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Default CC mode charging : %d, bNSC=%d\r\n", g_temp_CC_value, bNSC);
...
static kal_uint32 charging_full_check(void) //if (BMT_status.ICharging <= charging_full_current) & 6 times continuously->return KAL_TRUE, others->return KAL_FALSE
{
	...
	if(BMT_status.ICharging <= charging_full_current)
    {
    	full_check_count++;
//CEI comments start//
	   	    battery_xlog_printk(BAT_LOG_CRTI, "[charging_full_check] full_check_count=%d ICharging=%d charging_full_current=%d\n", full_check_count, BMT_status.ICharging, charging_full_current);
//CEI comments end//
...
static void charging_current_calibration(void) //set g_I_SENSE_offset as 0
{
...
static void pchr_sw_cv_charing_current_check(void) //set ibat as ulc_cv_charging_current & enable charging
{	
...
static void pchr_turn_on_charging (void) //set CV & ibat & starting charging if (charging_enable == KAL_TRUE), disableing charging if (charging_enable == KAL_FALSE)
{
#if !defined(MTK_JEITA_STANDARD_SUPPORT) 
	BATTERY_VOLTAGE_ENUM cv_voltage;
#endif
    ...
    else
    {
        ...
        if (get_usb_current_unlimited()) {
            g_temp_CC_value = AC_CHARGER_CURRENT;
            battery_xlog_printk(BAT_LOG_CRTI, "USB_CURRENT_UNLIMITED, use AC_CHARGER_CURRENT\n" );
        }			
        else {
        if (g_bcct_flag == 1)
        {
			battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] select_charging_curret_bcct !\n");
            select_charging_curret_bcct();
        }
        else
        {
       	    battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] select_charging_current !\n");
        ...
        {    
		   ...
		   #endif	
           {
			...
			/* Set CV */
				...
				#endif
//CEI comments start//
				battery_xlog_printk(BAT_LOG_CRTI, "[pchr_turn_on_charging] isInternalCharger=%d\n", isInternalCharger);
				if(!isInternalCharger)
					cv_voltage = BATTERY_VOLT_04_200000_V;
//CEI comments end//
				battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE,&cv_voltage);
...
//CEI comments start//
static PMU_STATUS BAT_PreChargeModeAction(void) //YF
//CEI comments end//
{
...
//CEI comments start//
static PMU_STATUS BAT_ConstantCurrentModeAction(void) //YF
//CEI comments end//
{
...
PMU_STATUS BAT_TopOffModeAction(void) //YF
{
...
//CEI comments start//
static PMU_STATUS BAT_BatteryFullAction(void) //YF
//CEI comments end//
{
	kal_uint32 charging_enable = KAL_FALSE;
...
//CEI comments start//
static PMU_STATUS BAT_BatteryHoldAction(void) //if (BMT_status.bat_vol < TALKING_RECHARGE_VOLTAGE || g_call_state == CALL_IDLE)->set BMT_status.bat_charging_state = CHR_CC
//CEI comments end//
{
...
//CEI comments start//
static PMU_STATUS BAT_BatteryStatusFailAction(void) //YF
//CEI comments end//
{
	kal_uint32 charging_enable;
...
void mt_battery_charging_algorithm() //YF
{
	#if defined(MTK_PUMP_EXPRESS_SUPPORT)
