...
#include <mach/smp.h>
#if defined(CONFIG_ARM_PSCI) && (CONFIG_TRUSTONIC_TEE_SUPPORT)
...
static bool mrdump_enable = true;
...
static void aee_kdump_cpu_stop(void *arg, void *regs, void *svc_sp)
{
	...
	elf_core_copy_kernel_regs(&crash_record->cpu_regs[cpu], ptregs);
...
#if defined(CONFIG_ARM_PSCI) && (CONFIG_TRUSTONIC_TEE_SUPPORT)
static void mrdump_tee_callback(void)
{
	mdelay(2000);
	if (mrdump_cb != NULL) {
		struct mrdump_crash_record *crash_record = &mrdump_cb->crash_record;
		...
		for (i = 0; i < CONFIG_NR_CPUS; i++) {
			crash_save_cpu(&crash_record->cpu_regs[i], i);
		}
...
static void __mrdump_reboot_va(AEE_REBOOT_MODE reboot_mode, struct pt_regs *regs, const char *msg, va_list ap)
{
	...
	elf_core_copy_kernel_regs(&crash_record->cpu_regs[cpu], regs);
	...
	if (reboot_mode != AEE_REBOOT_MODE_KERNEL_OOPS) {
...
#elif defined(CONFIG_ARM_PSCI) && (CONFIG_TRUSTONIC_TEE_SUPPORT)

	crash_record->reboot_mode = reboot_mode;
	mt_secure_call(TBASE_SMC_AEE_DUMP, &mrdump_tee_callback, 0, 0);
	while (1) {
		printk("Unexpected return from TEE\n");
		cpu_relax();
	}
#else
