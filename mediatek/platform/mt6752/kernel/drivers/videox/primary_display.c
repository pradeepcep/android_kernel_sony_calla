...
int primary_display_save_power_for_idle(int enter, unsigned int need_primary_lock)
{
	...
	static unsigned int isIdlePowerOff = 0;
...
static void _cmdq_build_Decouple_trigger_loop(void)
{
	int ret = 0;
	...
	if(primary_display_is_video_mode())
	{
       ...
       ret = cmdqRecWait(pgc->cmdq_handle_trigger, dpmgr_path_get_mutex(pgc->dpmgr_handle) + CMDQ_EVENT_MUTEX0_STREAM_EOF);
        // can not be b4 bit
       // cmdqBackupReadSlot(pgc->rdma_buff_info, 0, DISP_REG_RDMA_MEM_START_ADDR);
	}
	else
	{
...
{	
	...
	DISPMSG("dl_to_dc capture:Flush add memout mva(0x%x)\n",p_wdma->dstAddress);
                                    ...
                                    unsigned int data)
{
    ...
    MMProfileLogEx(ddp_mmp_get_events()->ovl_trigger, MMProfileFlagPulse, 0, (unsigned int)mem_config.addr);
}
                                    ...
                                    unsigned int data)
{
    ...
    cmdqRecReset(cmdq_handle);
    cmdqRecWait(cmdq_handle, CMDQ_EVENT_DISP_WDMA0_EOF);
    MMProfileLogEx(ddp_mmp_get_events()->ovl_trigger, MMProfileFlagPulse, mem_config.buff_idx, (unsigned int)mem_config.addr);
}
...
void primary_display_switch_esd_mode(int mode) //Taylor--20150108
{
    DISPFUNC();
	//int ret=0; //Taylor--20150108
	int gpio_mode=0;
...
static int _olv_wdma_fence_release_callback(uint32_t userdata)
{
    ...
    /* async callback,need to check if it is still decouple*/
    _primary_path_lock(__func__);
    if(_is_decouple_mode(pgc->session_mode) && userdata == DISP_SESSION_DECOUPLE_MIRROR_MODE)
    {
        static cmdqRecHandle cmdq_handle = NULL;
		unsigned int rdma_pitch_sec;
        if(cmdq_handle == NULL)
...
int primary_display_switch_mode(int sess_mode, unsigned int session, int force)
{
  ...
  DISPCHECK("primary_display_switch_mode sess_mode %d, session 0x%x\n",sess_mode,session);
	...
	if(pgc->session_mode == sess_mode) //mtk modify
	{
		goto done;
	}
	...
	if(pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE &&  sess_mode == DISP_SESSION_DECOUPLE_MODE)
	{
		...
		MMProfileLogEx(ddp_mmp_get_events()->primary_switch_mode, MMProfileFlagPulse, pgc->session_mode, sess_mode);
		//primary_display_diagnose();
    } else if(pgc->session_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE && sess_mode == DISP_SESSION_DIRECT_LINK_MIRROR_MODE){
        /*dc mirror  to dl mirror*/
		_DC_switch_to_DL_fast();
        pgc->session_mode = sess_mode;
		DISPMSG("primary display is %s mode now\n", session_mode_spy(pgc->session_mode));
		MMProfileLogEx(ddp_mmp_get_events()->primary_switch_mode, MMProfileFlagPulse, pgc->session_mode, sess_mode);
		//primary_display_diagnose();
    } else if(pgc->session_mode == DISP_SESSION_DIRECT_LINK_MIRROR_MODE && sess_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE){
        /*dc mirror  to dl mirror*/
		_DL_switch_to_DC_fast();
        pgc->session_mode = sess_mode;
		DISPMSG("primary display is %s mode now\n", session_mode_spy(pgc->session_mode));
...
static int primary_display_capture_framebuffer_immediate(unsigned long pbuf, unsigned int format)
{	
	unsigned int i =0;
	int ret =0;
	cmdqRecHandle cmdq_handle = NULL;
...
out:
	...
	DISPMSG("primary_display_capture_framebuffer_immediate: end\n");
...
int primary_display_capture_framebuffer_ovl(unsigned long pbuf, unsigned int format)
{
	...
	if(_is_decouple_mode(pgc->session_mode) || _is_mirror_mode(pgc->session_mode))
    {   
		primary_display_capture_framebuffer_immediate(pbuf, format);
		DISPMSG("primary capture: decouple mode\n");
        goto out;
    }
