...
//#if 0 // BAM_S for solve issue of illeagl headset judgement
//#else
//		if ( dwChannel == AUX_VACCDET_AP) {
//			msleep(5);
//		}
//#endif // BAM_E
...
//			dwChannel, ret_data);
//MTK add for 17degC start//
		xlog_printk(ANDROID_LOG_ERROR,"Power/PMIC", "[AUXADC] ruo output raw data[%d]=%d   u4channel=%d\n",dwChannel, ret_data,u4channel);
//MTK add for 17degC end//
		if (chip_num == MT6325_CHIP) {
			if (user_num == MD) {
				//Prerequisite settings before accessing AuxADC 
...
//MTK add for 17degC start//
	xlog_printk(ANDROID_LOG_ERROR,"Power/PMIC", "[AUXADC] ruo output average data[%d]=%d deCount = %d  u4channel=%d\n",dwChannel, adc_result_temp, deCount, u4channel);
//MTK add for 17degC end//
	
	switch(dwChannel){
	case AUX_BATSNS_AP:
//MTK mod for 17degC start//
		if (Enable_BATDRV_LOG == 3) {
//MTK mod for 17degC end//
...
//MTK add for 17degC start//
		xlog_printk(ANDROID_LOG_ERROR,"Power/PMIC", "[AUXADC] ruo output voltage batsense data[%d]=%d   r_val_temp=%d\n",dwChannel, adc_result,r_val_temp);
//MTK add for 17degC end//
		break;
	case AUX_ISENSE_AP:
//MTK mod for 17degC start//
