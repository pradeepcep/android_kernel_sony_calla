...
void write_magic(volatile unsigned long magic_write, int log_option)
{
	if (magic_base != 0)
	{
		writel(0xAD98, IOMEM(magic_base + 0xA0));
		writel(readl(IOMEM(magic_base + 0xA8)) | 0x80, IOMEM(magic_base + 0xA8));
		writel(0x0, IOMEM(magic_base + 0xA0));
		writel(magic_write, IOMEM(magic_base + 0x9C));		

		if( magic_write  == readl(IOMEM(magic_base + 0x9C)))
		{
			if(log_option == 1)
				spm_crit2("%x write_magic success !!\n", magic_write);
			else
				printk("%x write_magic success !!\n", magic_write);
		}
		else
		{
			if(log_option == 1)
				spm_crit2("%x write_magic failed !!\n", magic_write);
			else
				printk("%x write_magic failed !!\n", magic_write);
		}
	}
	else
	{
