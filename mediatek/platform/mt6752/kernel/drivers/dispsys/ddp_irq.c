...
#define MEM_DCM_CTRL  ((unsigned int *)0xf0001074)
#define DFS_DCM_CTRL  ((unsigned int *)0xf0001078)

void emi_bw_init(void)
{
     printk(KERN_ALERT "emi_bw_init: \n");
     // Disable EMI DCM
        *MEM_DCM_CTRL &= ~(1<<7);   // Disable 3PLL mode EMI DCM
        *MEM_DCM_CTRL |= 0x1;
        *MEM_DCM_CTRL &= ~(0x1);
        *DFS_DCM_CTRL &= ~(1<<7);   // Disable 1PLL mode EMI DCM
        *DFS_DCM_CTRL |= 0x1;
        *DFS_DCM_CTRL &= ~(0x1);

        // Enable EMI bus monitor
      writel(readl(EMI_CONM)|0x1<<30, EMI_CONM);
      writel(0x00FF0000, EMI_BMEN);
      writel(0x00200001, EMI_MSEL);
      writel(0x00000040, EMI_MSEL2);
      writel(0x02000000, EMI_BMEN2);
      //writel(0x55555555, EMI_BMRW0); 
      mt_reg_sync_writel(readl(EMI_BMEN)|0x1, EMI_BMEN);

}
void emi_bw_dump(void)
{
        writel(readl(EMI_BMEN)|0x2, EMI_BMEN);
        printk(KERN_ALERT "emi_bw_dump start: \n");
        printk(KERN_ALERT "0xf0001098 = 0x%x\n", readl(0xF0001098));
        printk(KERN_ALERT "Total BW(EMI_WSCT) = 0x%x\n", readl(EMI_WSCT));
        printk(KERN_ALERT "APMCU BW(EMI_WSCT2) = 0x%x\n", readl(EMI_WSCT2));
        printk(KERN_ALERT "MM BW(EMI_WSCT3) = 0x%x\n", readl(EMI_WSCT3));
        printk(KERN_ALERT "GPU BW(EMI_WSCT4) = 0x%x\n", readl(EMI_WSCT4));
        if(readl(EMI_TTYPE9) == 0)
                printk(KERN_ALERT "EMI_TTYPE1 = 0x%x, EMI_TTYPE9 = 0x%x, APMCU(M0)\n", readl(EMI_TTYPE1), readl(EMI_TTYPE9));
        else
                printk(KERN_ALERT "EMI_TTYPE1 = 0x%x, EMI_TTYPE9 = 0x%x, APMCU(M0) read latency = 0x%x\n", readl(EMI_TTYPE1), readl(EMI_TTYPE9), readl(EMI_TTYPE1) / readl(EMI_TTYPE9));
        if(readl(EMI_TTYPE12) == 0)
                printk(KERN_ALERT "EMI_TTYPE4 = 0x%x, EMI_TTYPE12 = 0x%x, MDMCU(M3)\n", readl(EMI_TTYPE4), readl(EMI_TTYPE12));
        else
                printk(KERN_ALERT "EMI_TTYPE4 = 0x%x, EMI_TTYPE12 = 0x%x, MDMCU(M3) read latency = 0x%x\n", readl(EMI_TTYPE4), readl(EMI_TTYPE12), readl(EMI_TTYPE4) / readl(EMI_TTYPE12));
        if(readl(EMI_TTYPE13) == 0)
                printk(KERN_ALERT "EMI_TTYPE5 = 0x%x, EMI_TTYPE13 = 0x%x, MDHW(M4)\n", readl(EMI_TTYPE5), readl(EMI_TTYPE13));
        else
                printk(KERN_ALERT "EMI_TTYPE5 = 0x%x, EMI_TTYPE13 = 0x%x, MDHW(M4) read latency = 0x%x\n", readl(EMI_TTYPE5), readl(EMI_TTYPE13), readl(EMI_TTYPE5) / readl(EMI_TTYPE13));
        if(readl(EMI_TTYPE14) == 0)
                printk(KERN_ALERT "EMI_TTYPE6 = 0x%x, EMI_TTYPE14 = 0x%x, MM(M5)\n", readl(EMI_TTYPE6), readl(EMI_TTYPE14));
        else
                printk(KERN_ALERT "EMI_TTYPE6 = 0x%x, EMI_TTYPE14 = 0x%x, MM(M5) read latency = 0x%x\n", readl(EMI_TTYPE6), readl(EMI_TTYPE14), readl(EMI_TTYPE6) / readl(EMI_TTYPE14));
        if(readl(EMI_TTYPE15) == 0)
                printk(KERN_ALERT "EMI_TTYPE7 = 0x%x, EMI_TTYPE15 = 0x%x, GPU(M6\n", readl(EMI_TTYPE7), readl(EMI_TTYPE15));
        else
                printk(KERN_ALERT "EMI_TTYPE7 = 0x%x, EMI_TTYPE15 = 0x%x, GPU(M6) read latency = 0x%x\n", readl(EMI_TTYPE7), readl(EMI_TTYPE15), readl(EMI_TTYPE7) / readl(EMI_TTYPE15));
}
    ...
    //switch(irq)
    {
        ...
        else if(irq==dispsys_irq[DISP_REG_RDMA0] || irq==dispsys_irq[DISP_REG_RDMA1])
		{
                ...
                if(reg_val&(1<<4))
                {
                      ...
                      if(gEnableUnderflowAEE)
                      {
                          ddp_aee_print("rdma%d underflow %d! \n", index, rdma_underflow_irq_cnt[index]);
                      }
                }
                if(reg_val&(1<<5))
                {
