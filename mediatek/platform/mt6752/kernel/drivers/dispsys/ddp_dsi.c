...
void ddp_dsi_clk_off(DISP_MODULE_ENUM module, void *cmdq_handle,unsigned int level)//Taylor--20150108
{
	int ret=0;
...
int ddp_dsi_ioctl(DISP_MODULE_ENUM module, void *cmdq_handle, unsigned int ioctl_cmd, unsigned long *params)
{
	...
	switch(ioctl)
	{
		...
		case DDP_DSI_IDLE_CLK_CLOSED:
		{
			...
			printk("[ddp_dsi_ioctl_close] level = %d\n", idle_cmd);
			if(idle_cmd==0)
			{
		...
		{
			unsigned int idle_cmd = params[0];
			printk("[ddp_dsi_ioctl_open] level = %d\n", idle_cmd);
			if(idle_cmd==0)
			{
