...
static int mtk_wdt_probe(struct platform_device *dev)
{
	...
	iram_node = of_find_compatible_node(NULL, NULL, "mediatek,SRAMROM");
	...
	magic_val = readl(IOMEM(magic_base + 0x9C));

	printk("magic addr= 0x%x, magic val = 0x%x\n", magic_base+0x9C, magic_val);
#endif
