...
//CEI comments start//
    ...
    if(p == NULL)
        printk("Can't find hwid cmdline\n");
    else
    {
        p += 5;
        iHwId = simple_strtol(p, NULL, 10);
        iIsPdp1 = (iHwId == 1) ? 1 : 0;
        Printhh("[%s] iIsPdp1 = %d, iHwId=%d\n", __FUNCTION__, iIsPdp1, iHwId);
    }
#endif
//CEI comments end//
...
//        md_sim_info[md_sim_counter].eint_pol= CUST_EINT_MD1_0_POLARITY;
        if(iIsPdp1)
        {
            //PDP1
            Printhh("[%s] PDP1..\n", __FUNCTION__);
            md_sim_info[md_sim_counter].eint_pol= CUST_EINT_POLARITY_HIGH;
        }
        else{
            //not PDP1
            Printhh("[%s] notPDP1..\n", __FUNCTION__);
        ...
        Printhh("[%s] md_sim_info[md_sim_counter].eint_pol = %d..\n", __FUNCTION__, md_sim_info[md_sim_counter].eint_pol);
