...
//[LX16] ==> CCI KLog, added by Jimmy@CCI
...
#if defined(CONFIG_CCI_PM_WAKELOCK_LOG) || defined(CONFIG_CCI_PM_WAKEUP_LOG)
#ifdef CONFIG_CCI_PM_WAKELOCK_LOG
#include <linux/slab.h>
static char *lock_buffer = NULL;
#endif // #ifdef CONFIG_CCI_PM_WAKELOCK_LOG
static void stay_awake_reason_work_func(struct work_struct *work);
static DECLARE_DELAYED_WORK(stay_awake_reason_work, stay_awake_reason_work_func);
#endif // #if defined(CONFIG_CCI_PM_WAKELOCK_LOG) || defined(CONFIG_CCI_PM_WAKEUP_LOG)
static suspend_state_t autosleep_state = PM_SUSPEND_BOOTING;
...
//wakelock
	...
	if(!lock_buffer)
	{
		lock_buffer = kmalloc(PAGE_SIZE, GFP_KERNEL);
	}
	if(lock_buffer)
	{
		memset(lock_buffer, 0, PAGE_SIZE);
		pm_show_wakelocks(lock_buffer, true);
		if(lock_buffer[strlen(lock_buffer) - 1] == '\n')
		{
			lock_buffer[strlen(lock_buffer) - 1] = 0;
		}
		if(strlen(lock_buffer) > 0)
		{
			printk("[WAKELOCK]%s\n", lock_buffer);
		}
	}
	...
	{
		printk("[WAKELOCK]allocate buffer failed\n");
	}
#endif // #ifdef CONFIG_CCI_PM_WAKELOCK_LOG

//wakeup source
#ifdef CONFIG_CCI_PM_WAKEUP_LOG
	print_active_wakeup_source();
#endif // #ifdef CONFIG_CCI_PM_WAKEUP_LOG

	schedule_delayed_work(&stay_awake_reason_work, msecs_to_jiffies(CONFIG_CCI_PM_ACTIVE_STAY_AWAKE_LOG_PERIOD * 1000) + 1);
}
#endif // #if defined(CONFIG_CCI_PM_WAKELOCK_LOG) || defined(CONFIG_CCI_PM_WAKEUP_LOG)
//[LY28] <== power related log, added by Jimmy@CCI
...
//[LY28] ==> power related log, added by Jimmy@CCI
#ifdef CONFIG_CCI_PM_LOG
	printk("[AUTOSLEEP]set state from %d to %d\n", autosleep_state, state);
#endif // #ifdef CONFIG_CCI_PM_LOG
//[LY28] <== power related log, added by Jimmy@CCI
	autosleep_state = state;
...
//[LY28] ==> power related log, added by Jimmy@CCI
#if defined(CONFIG_CCI_PM_WAKELOCK_LOG) || defined(CONFIG_CCI_PM_WAKEUP_LOG)
		schedule_delayed_work(&stay_awake_reason_work, msecs_to_jiffies(1));
#endif // #if defined(CONFIG_CCI_PM_WAKELOCK_LOG) || defined(CONFIG_CCI_PM_WAKEUP_LOG)
//[LY28] <== power related log, added by Jimmy@CCI
...
//[LY28] ==> power related log, added by Jimmy@CCI
#if defined(CONFIG_CCI_PM_WAKELOCK_LOG) || defined(CONFIG_CCI_PM_WAKEUP_LOG)
		cancel_delayed_work_sync(&stay_awake_reason_work);
#endif // #if defined(CONFIG_CCI_PM_WAKELOCK_LOG) || defined(CONFIG_CCI_PM_WAKEUP_LOG)
//[LY28] <== power related log, added by Jimmy@CCI
