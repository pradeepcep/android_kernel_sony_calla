...
//CEI comments start
//[BugID:322]:2014/10/21 Jiahan Li: Add file node and tune the procedure of brightness and blink.
...
//CEI comments start
//[BugID:480]2014/11/13 Jiahan Li: Circuit change for AP build.
...
{
    //CEI comments start
    //[BugID:522]2014/11/17 Jiahan Li: Red LED keeps on after a notification blinking.
	int hw_id = get_cci_hw_id() & 0x07;
    //CEI comments end
    //CEI comments start
    //[BugID:480]2014/11/13 Jiahan Li: Circuit change for AP build.
...
static int __init mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
        ...
        if (strcmp(g_leds_data[i]->cdev.name,"red") == 0) {
            ...
            //CEI comments start
            //[BugID:479]2014/11/13 Jiahan Li: Brightness level does not work.             
			rc = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_blink_brightness);
	        if (rc)
		        goto err;
            //CEI comments end
		}
