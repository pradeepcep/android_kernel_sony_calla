...
//		obj->dev_list.data_dev[MAX_ANDROID_SENSOR_NUM].get_data);
	...
	{
		BATCH_LOG("[MAX_ANDROID_SENSOR_NUM].flush != NULL \n");
//CEI comments start hh//
// fix CTS fail issue.
//		if((obj->active_sensor & (0x01<< handle)))
		{
			...
			*/
			...
			BATCH_LOG("batch mode is not enabled for this sensor! 1 \n");
			//obj->flush_result= -1;    //henry tt
			obj->flush_result= 0;
			return;
		}
#endif
//CEI comments end//
	...
	if((obj->active_sensor & (0x01<< handle))){
		if((obj->dev_list.ctl_dev[handle].flush != NULL) && (obj->dev_list.data_dev[handle].get_data != NULL))
		{
			...
			BATCH_LOG("batch mode is not enabled for this sensor! 2 \n");
			obj->flush_result = -1;
			return;
		}
	}
	else{
			BATCH_LOG("batch mode is not enabled for this sensor! 3 \n");
			obj->flush_result= -1;
			return;
		}
	obj->flush_result = 0;
