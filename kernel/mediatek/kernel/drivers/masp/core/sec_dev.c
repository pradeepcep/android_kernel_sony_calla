...
#define BOOT_REGION1_PATH                   "/dev/block/mmcblk0boot1"
 ...
 *  READ INFO
 **************************************************************************/
extern int simple_sd_get_bootpart(struct msdc_ioctl *msdc_ctl);
static int get_bootpart(unsigned int *bootpart)
{
    int ret = 0;
    struct msdc_ioctl cmd = {0};

    cmd.buffer = bootpart;
    cmd.opcode = MSDC_CARD_DUNM_FUNC;

    ret = simple_sd_get_bootpart(&cmd);
    if (ret) {
        *bootpart = 0;
        ret = -EIO;
    }
...
int read_info (int part_index, uint32 part_off, uint32 search_region, char* info_name, uint32 info_name_len, uint32 info_sz, char* info_buf)
{
    ...
    if ((MAX_MTD_PARTITIONS + 1) == part_index)
    {
        ...
        /* get active boot partition to locate active preloader */
        ret = get_bootpart(&bootpart);    
        if (ret)
            goto _end;
        ...
        if (bootpart == 1) {
            ...
            mcpy(part_path,BOOT_REGION1_PATH,strlen(BOOT_REGION1_PATH));
            part_path[strlen(BOOT_REGION1_PATH)] = '\0';
        }
        else {
            /* read boot1 by default */
            mcpy(part_path,BOOT_REGION0_PATH,strlen(BOOT_REGION0_PATH));
            part_path[strlen(BOOT_REGION0_PATH)] = '\0';
        }
    }
