...
void rdma_set_ultra(unsigned int idx, unsigned int width, unsigned int height, unsigned int bpp, unsigned int frame_rate,void * handle)
{
...
#if 0  // this setting will cause underflow in 720P user load, so rollback to #else's setting
  if(idx==0)
  {
      if(width>=1080)
      {
  ...
  else
  {
    ...
    if(idx==0)
    {
        //add for 1536 * 2048 panel
        if(width>=1536)
        {
            DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x1b013b96);
        }
        else
        {
            DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x1b01356b);
...
#else
    DDPDBG("RDMAConfig idx %d, mode %d, address 0x%x, inputformat %s, pitch %u, width %u, height %u,sec%d\n",
                         idx, mode, address, rdma_intput_format_name(inputFormat,input_swap), pitch,width, height, sec);
#endif
    ASSERT(idx <= 2);
	...
	if(mode == RDMA_MODE_MEMORY) {
		if(security == DISP_SECURE_BUFFER) {
			...
			if(rdma_is_sec[rdma_idx]) {
				...
				if(ret)
					ddp_aee_print("[SVP]fail to create disable handle %s ret=%d\n", __FUNCTION__, ret);
