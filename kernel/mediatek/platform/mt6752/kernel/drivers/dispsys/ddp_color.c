...
static int _color_clock_on(DISP_MODULE_ENUM module, void *cmq_handle)
{
    ...
    {
        enable_clock(MT_CG_DISP0_DISP_COLOR , "color0");
        COLOR_DBG("color[0]_clock_on CG 0x%x \n",DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON0));
    }
    else
    {
        enable_clock(MT_CG_DISP0_DISP_COLOR , "color1");
...
static int _color_clock_off(DISP_MODULE_ENUM module, void *cmq_handle)
{
    ...
    if (module == DISP_MODULE_COLOR0)
    {
        ...
        disable_clock(MT_CG_DISP0_DISP_COLOR, "color0");
    }
    else
    {
        COLOR_DBG("color[1]_clock_off\n");
        disable_clock(MT_CG_DISP0_DISP_COLOR, "color1");
    }
#endif
    return 0;
}
...
static int _color_io(DISP_MODULE_ENUM module, int msg, unsigned long arg, void *cmdq)
{
    ...
    switch (msg) {
        ...
        case DISP_IOCTL_READ_REG:
        {
            ...
            {
                COLOR_ERR("reg read, addr invalid, addr=0x%lx \n", rParams.reg);
                return -EFAULT;
            }
        ...
        case DISP_IOCTL_WRITE_REG:
        {
            ...
            if(ret == 0)
            {
                COLOR_ERR("reg write, addr invalid, addr=0x%lx \n", wParams.reg);
                return -EFAULT;
            }
...
void set_color_bypass(DISP_MODULE_ENUM module, int bypass, void* cmdq_handle)
{
    ...
    if (bypass)
    {
        ...
        DISP_REG_SET(cmdq_handle, DISP_COLOR_CFG_MAIN + offset,(1<<7)); // all bypass
        DISP_REG_SET(cmdq_handle, DISP_COLOR_START + offset, 0x00000003); //color start
    }
    else
    {
        DISP_REG_SET(cmdq_handle, DISP_COLOR_CFG_MAIN + offset,(0<<7)); //color enable
        DISP_REG_SET(cmdq_handle, DISP_COLOR_START + offset, 0x00000001); //color start

        // enable R2Y/Y2R in Color Wrapper
        DISP_REG_SET(cmdq_handle, DISP_COLOR_CM1_EN + offset, 0x01);
        DISP_REG_SET(cmdq_handle, DISP_COLOR_CM2_EN + offset, 0x11);   // also set no rounding on Y2R
    }
}
#endif
