...
//Add for handle oemS and oemF by adb reboot command
#ifdef CONFIG_SONY_S1_SUPPORT
	else if (!strcmp(cmd, "oemS")) {
		spar0 = rtc_read(RTC_SPAR0) & (~RTC_SPAR0_S1_SERVICE);
		rtc_write(RTC_SPAR0, spar0 | RTC_SPAR0_S1_SERVICE);
	}
	else if (!strcmp(cmd, "oemF")) {
		spar0 = rtc_read(RTC_SPAR0) & (~RTC_SPAR0_FOTA);
		rtc_write(RTC_SPAR0, spar0 | RTC_SPAR0_FOTA);
	}
#endif // #ifdef CONFIG_SONY_S1_SUPPORT
//CEI comments end//
	rtc_write_trigger();
