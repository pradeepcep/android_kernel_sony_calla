...
int hw_id = 0;
unsigned char isInternalCharger = 1;
//CEI comments end//
...
if(isInternalCharger)
	...
	BQ24158_BATTERY_SetUSBState(usb_state);
//CEI comments end//
	wake_up_bat();
#endif
}
...
static int __init get_hw_id(char *str)
{
	char *pvalue = NULL;

	hw_id = simple_strtol(str, &pvalue, 10);
	hw_id &= 0x07;
	return 1;
}
...
//CEI comments start//
	__setup("hwid=", get_hw_id);
//	hw_id = get_cci_hw_id() & 0x07;
	isInternalCharger = (hw_id != 1) ? 1 : 0;
	printk("[mt_usb_probe] hw_id=%d, isInternalCharger=%d\n", hw_id, isInternalCharger);
//CEI comments end//
