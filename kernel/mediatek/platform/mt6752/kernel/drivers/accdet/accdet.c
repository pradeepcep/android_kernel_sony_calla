...
// BAM_S 20141204
static int LIVE_KEY_HIGH_THR;//BAM for LiveKey
static int DW_KEY_HIGH_THR; //0.50v=500000uv
static int DW_KEY_THR; //0.22v=220000uv
static int UP_KEY_THR; //0.08v=80000uv
static int MD_KEY_THR;
//BAM_E
...
#define MD_KEY		  	 (0x02)
#define DW_KEY			 (0x04)
#define LIVE_KEY 		 (0X08)//BAM for LiveKey
...
#define SHORT_LIVE				 ((LIVE_KEY) | SHORT_PRESS)//BAM for LiveKey
#define LONG_UP                  ((UP_KEY) | LONG_PRESS)
#define LONG_MD                  ((MD_KEY) | LONG_PRESS)
#define LONG_DW                  ((DW_KEY) | LONG_PRESS)
#define LONG_LIVE				 ((LIVE_KEY)| LONG_PRESS)// BAM for LiveKey
...
/*
        ...
        MD              UP                DW                    LIVE_KEY
|---------|-----------|----------|----------|
0V<=MD< 0.08V<= UP<0.22V<=DW <0.5V<=LIVE_KEY<550

*/
#if 0 // BAM_S 20141204
#define LIVE_KEY_HIGH_THR (550)//BAM for LiveKey
...
static int key_check(int b)
{
	...
	else if ((b < UP_KEY_THR) && (b >= MD_KEY_THR))
	{
		...
		return MD_KEY;
//BAM_S for LiveKey
	} else if ((b < LIVE_KEY_HIGH_THR) && (b >= DW_KEY_HIGH_THR)) {
		return LIVE_KEY;
	}
//BAM_E for LiveKey
	return NO_KEY;
...
//BAM_S for LiveKey
				case LIVE_KEY:
		   	        input_report_key(kpd_accdet_dev, BTN_3, flag);
                    input_sync(kpd_accdet_dev);
					ACCDET_DEBUG("KEY_LIVE %d\n",flag);
		   	        break;
//BAM_E for LiveKey
                }
       }
...
#if 0 // BAM_S 20141029
#else
			if (ACC_KEY_MEDIA== accdet_key_event) {
						   ACCDET_DEBUG("[Accdet] KEY_SEARCH!\n");
						   input_report_key(kpd_accdet_dev, KEY_SEARCH, 1);
						   input_report_key(kpd_accdet_dev, KEY_SEARCH, 0);
						   input_sync(kpd_accdet_dev);
			}
#endif // BAM_E
...
#if 0 // BAM_S for OMTP warning message
...
//BAM_S for LiveKey
			case SHORT_LIVE:
				ACCDET_DEBUG("[Accdet] Short press down live key(0x%x)\n", multi_key);
				break;
//BAM_E for LiveKey
			case LONG_UP:
...
#else
								if(call_status == 0) {
								notify_sendKeyEvent(ACC_KEY_MEDIA);
								} else {
									notify_sendKeyEvent(ACC_END_CALL);
								}
#endif // BAM_E
...
//BAM_S for LiveKey
			case LONG_LIVE:
				ACCDET_DEBUG("[Accdet] Long press down live key(0x%x)\n",multi_key);
				break;
//BAM_E for LiveKey
			default:
...
{ 
   ...
   #ifdef ACCDET_PIN_RECOGNIZATION
    pmic_pwrap_write(ACCDET_DEBOUNCE0, cust_headset_settings->debounce0);
#if 0 // BAM_S 20141104 DMS06039650
	   pmic_pwrap_write(ACCDET_DEBOUNCE1, 0xFFFF);
#else
	   pmic_pwrap_write(ACCDET_DEBOUNCE1, 0x2000);//delay 250ms=0x2000/32768
#endif // BAM_E
...
#if 1 // BAM_S 20141204
	p = strstr(saved_command_line, "hwid=");
	if(p == NULL) {
		ACCDET_DEBUG("[Accdet] Can't find hwid cmdline\n");
		LIVE_KEY_HIGH_THR = 600;
		DW_KEY_HIGH_THR = 370;
		DW_KEY_THR = 190;
		UP_KEY_THR = 50;
		MD_KEY_THR = 0;
		...
		p += 5;
		HwId = simple_strtol(p, NULL, 10);
		ACCDET_DEBUG("[Accdet] HwId=%d\n", HwId);
		if (HwId == HwId_SP || HwId == HwId_PDP1 || HwId == HwId_PDP2) {
			LIVE_KEY_HIGH_THR = 550;//BAM for LiveKey
			DW_KEY_HIGH_THR = 500; //0.50v=500000uv
			DW_KEY_THR = 220; //0.22v=220000uv
			UP_KEY_THR = 80; //0.08v=80000uv
			MD_KEY_THR = 0;
		} else {
			//AP:2 PQ:6
			LIVE_KEY_HIGH_THR = 600;
...
//BAM_E for LiveKey
//BAM_S 20141031 for middle key long press
	__set_bit(KEY_SEARCH,kpd_accdet_dev->keybit);
//BAM_E for middle key long press
