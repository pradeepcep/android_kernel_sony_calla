...
#define GETARRAYNUM(array) (sizeof(array)/sizeof(array[0]))
 ...
 // ============================================================ //
//CEI comments start//
// kal_bool chargin_hw_init_done = KAL_TRUE; //determined by bq24158_init()
//CEI comments end//
...
//CEI comments end//
//Transfer to parameter value from register value
{
    if (val < array_size)
    {
...
//CEI comments end//
//Transfer to register value from parameter value
{
    kal_uint32 i;
...
//MTK add for 17degC start//
    mt6325_upmu_set_rg_csdac_mode(1);      //CSDAC_MODE
//MTK add for 17degC end//    
    mt6325_upmu_set_rg_vbat_ov_en(1);      //VBAT_OV_EN
//CEI comments start//
    battery_xlog_printk(BAT_LOG_CRTI, "[charging_hw_init] isInternalCharger=%d\n", isInternalCharger);
//CEI comments end//
...
static kal_uint32 charging_get_current(void *data) //get ibat
{
    kal_uint32 status = STATUS_OK;
...
//MTK mod for 17degC start//
//CEI comments start//
//    pmic_read_interface(MT6325_CHR_CON0,&reg_value,0xF,0);//RG_CS_VTH //MTK-ORG
    pmic_read_interface(MT6325_CHR_CON4,&reg_value,0xF,0);//RG_CS_VTH
    battery_xlog_printk(BAT_LOG_CRTI, "ibat_reg=0x%x\r\n", reg_value);
//CEI comments end//
//MTK mod for 17degC end//
...
static kal_uint32 charging_set_current(void *data) //set ibat
{
    kal_uint32 status = STATUS_OK;
...
//CEI comments start//
    battery_xlog_printk(BAT_LOG_CRTI, "YF-ibat=%d\r\n", (*(kal_uint32 *)data));
//CEI comments end//
    array_size = GETARRAYNUM(CS_VTH);
...
static kal_uint32 charging_set_input_current(void *data) //set iusb
{
    kal_uint32 status = STATUS_OK;
//CEI comments start//
    battery_xlog_printk(BAT_LOG_CRTI, "YF-iusb=%d\r\n", (*(kal_uint32 *)data));
//CEI comments end//
    return status;
}     
...
//CEI comments start//
    battery_xlog_printk(BAT_LOG_CRTI, "YF-VCDT=%d\r\n", voltage);
//CEI comments end//
...
//CEI comments start//
static kal_uint32 charging_get_battery_status(void *data) //return battery if exist, 0->exist, others->don't exist
{
    kal_uint32 status = STATUS_OK;
    kal_uint32 val = 0;
...
static kal_uint32 charging_get_charger_det_status(void *data) //return charger if exist, 1->exist, others->don't exist
{
    kal_uint32 status = STATUS_OK;
...
static kal_uint32 charging_get_charger_type(void *data) //return charger type, refer to enum CHARGER_TYPE
{
    kal_uint32 status = STATUS_OK;
...
//return if PCM timer wake up, true->is PCM timer wake up, false->not PCM timer wake up
{
     kal_uint32 status = STATUS_OK;
 ...
 static kal_uint32 charging_set_platform_reset(void *data) //execute platform reset
 {
     kal_uint32 status = STATUS_OK;
 ...
 static kal_uint32 charging_get_platfrom_boot_mode(void *data) //return platform boot mode, refer to enum BOOTMODE
 {
     kal_uint32 status = STATUS_OK;
...
static kal_uint32 charging_set_power_off(void *data) //execute power off
{
    kal_uint32 status = STATUS_OK;
...
static kal_uint32 charging_get_power_source(void *data) //always return false
{
    kal_uint32 status = STATUS_OK;
...
static kal_uint32 charging_get_csdac_full_flag(void *data) //always return false
{
    kal_uint32 status = STATUS_OK;
    *(kal_bool *)data = KAL_FALSE;
	return status;	
...
//set ta current pattern as increase(data is true) or decrease(data is false)
//return 0->success, others->fail
{
	...
	battery_xlog_printk(BAT_LOG_CRTI, "[charging_set_ta_current_pattern] increase=%d count=%d debug_val=0x%x\n", increase, count, debug_val);
