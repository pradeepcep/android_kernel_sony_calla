...
//CEI comments start//
	battery_xlog_printk(BAT_LOG_CRTI, "[bq24158_init] isInternalCharger=%d\n", isInternalCharger);
	if(isInternalCharger)
	{
	chargin_hw_init_done = KAL_TRUE;
	battery_xlog_printk(BAT_LOG_CRTI, "[bq24158_init] Using internal charger IC\n");
	return 0;
	}
	battery_xlog_printk(BAT_LOG_CRTI, "[bq24158_init] Using BQ24158 charger IC\n");
//CEI comments end//
    battery_xlog_printk(BAT_LOG_CRTI,"[bq24158_init] init start\n");
    
    i2c_register_board_info(0, &i2c_bq24158, 1);
...
//CEI comments start//
	if(isInternalCharger)
	{
	battery_xlog_printk(BAT_LOG_CRTI, "[bq24158_exit] Using internal charger IC\n");
	return;
	}
	battery_xlog_printk(BAT_LOG_CRTI, "[bq24158_exit] Using BQ24158 charger IC\n");
//CEI comments end//
