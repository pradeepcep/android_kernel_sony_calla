...
//SD Overheat Issue
    mt6325_upmu_set_rg_int_en_ldo_oc(1); //change to enable oc int for Vmch
    mt6325_upmu_set_rg_vmch_ocfb_en(1); //add as SY suggest
...
void ldo_int_handler(void)
{
    ...
    //sub-task
//SD Overheat Issue
    if(1 == mt6325_upmu_get_oc_status_vmch())
         msdc_sd_power_off();
...
//CEI comments start//
#define PLUG_UNPLUG_INTERVAL		20
#define PLUG_UNPLUG_COUNT			5
...
//CEI comments start//
	...
	if(LastUnplugTime)
	{
		if(abs(UnplugTime-LastUnplugTime) < PLUG_UNPLUG_INTERVAL)
			UnplugCount++;
		else
		{
			UnplugCount = 0;
...
//CEI comments start//
	...
	{
		if(UnplugCount >= PLUG_UNPLUG_COUNT)
		{
			bNSC = KAL_TRUE;
			UnplugTime = 0;
			LastUnplugTime = 0;
	...
	battery_xlog_printk(BAT_LOG_CRTI,"[chrdet_int_handler] UnplugTime=%d, UnplugCount=%d, bNSC=%d\n", UnplugTime, UnplugCount, bNSC);
//CEI comments end//
    #endif
    #else
    upmu_get_rgs_chrdet();
