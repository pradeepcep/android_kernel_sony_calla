...
//CEI comments start//
//Add for TP simulate P-sensor
int tp_special_flag = -1;
EXPORT_SYMBOL(tp_special_flag);

static int __init cci_info_check(char *vu)
{
    if(vu != NULL)
    {
        if(vu[0] == '1')
    ...
    printk("info_check: vu[0] = %c, tp_special_flag=%d\n", vu[0], tp_special_flag);
	
    return 1;
}
__setup("vu=", cci_info_check);
//CEI comments end//
